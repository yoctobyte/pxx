...
DEMOS=(
  ...
  "life|examples/life/life.pas|gui|Conway's Game of Life (GTK window, Start/Stop button)"
  ...
  "calc|examples/calc/calcdemo.pas|batch|Expression calculator (fixed expressions checked against known results)"
  "lisp|examples/lisp/lispdemo.pas|batch|Lisp interpreter (runs a fixed program, checks the results)"
  ...
  "httpdemo|examples/net/httpdemo.pas|batch|Async HTTP server and client over loopback (no network needed)"
  ...
  "fm|examples/fm/fm.pas|batch|Terminal file browser (renders once; run with --interactive to browse)"
  "player|examples/player/player.pas|tty|Terminal video player (needs ffmpeg: player <video-file>)"
)
