...
| `--emit-obj` | Emit a relocatable object (`.o`) instead of a linked executable. `x86-64`, `riscv32` and `xtensa` write a general object — code, data, bss, relocations, exported symbols — for a compiled source (Pascal, C or NilPy); `.asm` sources produce objects on x86-64 only. `i386`, `aarch64` and `arm32` have no object writer. On x86-64 the export surface is the C-convention routines and a link needs `-no-pie` — see [Linking a pxx object into another program](./objects.md). Same as an output path ending in `.o`. |
