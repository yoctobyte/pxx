# Progress board

_Generated by `tools/progress.sh board-md` — regenerate after any board
change; `tools/progress.sh check` fails if this file is stale. History
lives in git, not in a timestamp._

| urgent (0) | working (0) | backlog (31) | blocked (1) | done (1) | rejected (0) |
| --- | --- | --- | --- | --- | --- |
|  |  | bug-c-packed-aligned-ignored | bug-nonreproducible-miscompile-2026-06-02 | idea-progress-board-md |  |
|  |  | bug-managed-byref-string-param-store |  |  |  |
|  |  | bug-operator-result-inferred-var |  |  |  |
|  |  | bug-read-preserves-line-remainder |  |  |  |
|  |  | bug-whole-record-copy-main-body-noop |  |  |  |
|  |  | chore-inc-to-units |  |  |  |
|  |  | chore-runtime-emission-size |  |  |  |
|  |  | feature-additional-cpu-targets |  |  |  |
|  |  | feature-async-coroutines |  |  |  |
|  |  | feature-c-header-import-complex |  |  |  |
|  |  | feature-compiler-warnings |  |  |  |
|  |  | feature-dynamic-soname-discovery |  |  |  |
|  |  | feature-flexcolumn-directive |  |  |  |
|  |  | feature-float-str-val |  |  |  |
|  |  | feature-inline-asm-depth |  |  |  |
|  |  | feature-interfaces |  |  |  |
|  |  | feature-lazycasing-c-imports |  |  |  |
|  |  | feature-managed-exception-cleanup |  |  |  |
|  |  | feature-managed-string-default |  |  |  |
|  |  | feature-metaclass-descendant-enforcement |  |  |  |
|  |  | feature-progress-check-makefile |  |  |  |
|  |  | feature-progress-transition-helpers |  |  |  |
|  |  | feature-static-arena-profile |  |  |  |
|  |  | feature-target-aarch64 |  |  |  |
|  |  | feature-target-arm32 |  |  |  |
|  |  | feature-target-esp32 |  |  |  |
|  |  | feature-target-i386 |  |  |  |
|  |  | feature-threadsafe-io-serialization |  |  |  |
|  |  | feature-unified-heap-allocator |  |  |  |
|  |  | idea-unit-rename-import |  |  |  |
|  |  | idea-visibility-enforcement |  |  |  |

## Ready (no unmet blocker)

- bug-c-packed-aligned-ignored
- bug-managed-byref-string-param-store
- bug-operator-result-inferred-var
- bug-read-preserves-line-remainder
- bug-whole-record-copy-main-body-noop
- chore-inc-to-units
- chore-runtime-emission-size
- feature-c-header-import-complex
- feature-compiler-warnings
- feature-dynamic-soname-discovery
- feature-float-str-val
- feature-inline-asm-depth
- feature-interfaces
- feature-managed-exception-cleanup
- feature-metaclass-descendant-enforcement
- feature-progress-check-makefile
- feature-progress-transition-helpers
- feature-target-i386
- feature-unified-heap-allocator
- idea-unit-rename-import
- idea-visibility-enforcement

## Leverage (tickets each one unblocks)

- **3** — feature-unified-heap-allocator
- **2** — feature-target-i386
- **2** — feature-target-arm32
- **2** — feature-target-aarch64
- **1** — feature-target-esp32
- **1** — feature-managed-exception-cleanup
- **1** — feature-compiler-warnings
- **1** — chore-inc-to-units
- **1** — bug-managed-byref-string-param-store
