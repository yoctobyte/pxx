...
- 2026-06-21 - DONE (Approach B, three commits, all self-host byte-identical +
  `make test` green + output-equal x86-64/i386/aarch64/arm32):
  - Slice 1 (c820e78): parse-accept + flatten. A nested routine is forward-
    declared in place (so the enclosing body's calls resolve) and its full token
    stream stashed (`PendNestTok`), re-injected after the enclosing routine
    (`FlushPendingNestedProcs`) to be parsed as a top-level sibling that
    completes the forward decl. No new per-target codegen.
  - Slice 2a (334d679): Self/field capture. Free-variable analysis over the body
    tokens at the decl point; implicit field/method refs (and explicit `Self`)
    rewritten through a `__nestself` param; call sites get `Self` appended.
  - Slice 2b (e299709): enclosing local/param capture. Each captured var is
    snapshotted (the enclosing Syms slot is recycled before the lifted body
    parses) into a lift descriptor and injected as a by-ref param in
    ParseSubroutine, matched by a mangled unique name `<name>$<tokpos>`. Captured
    dyn-arrays travel as by-ref handles (SetLength resizes the caller's array).
    Body refs to the own name (function-result var / recursion) follow the
    mangle. `NestScanSpans` is a top-level helper (NOT a nested routine) so the
    compiler still self-hosts.
  - LANDMINES: (1) param Syms slots are recycled across procs — capture must
    snapshot type metadata at the decl point, never store the Sym index for later
    read. (2) Mangling the routine name breaks the FPC `FuncName := expr` result-
    var idiom and recursion — rewrite own-name body refs to the mangled name too.
    (3) Do not introduce a nested routine into compiler source as part of this
    work: Slice 2b rejects fixed-array capture and >1-level capture, so a nested
    routine in the compiler's own source could fail self-host.
  - DEFERRED (diagnosed, not errors-of-omission): capture of a fixed-size array;
    capture-of-capture across >1 nesting level (re-threading); `@nested` /
    passing a nested routine as a proc-typed arg (escaping closures are unsound
    under B — the by-ref captures are only live while the enclosing frame is).
  - adventure: now compiles BOTH nested routines — `TGame.LoadWorld.AddExit`
    (Self/field capture, the original engine.pas:604 blocker) AND `TGame.Run.
    AddVerb` (captures local `verbs: array of TVerb`). Its remaining blocker is
    UNRELATED: calling a proc-pointer record field `v.Run(...)` — pre-existing on
    the pinned stable compiler (filed: bug-procptr-record-field-call).
