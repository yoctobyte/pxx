...
* **Reference behavior**: Heap-allocated arrays declared as `array of Type`. Resized at runtime via `SetLength(Arr, Size)`. Behind the scenes, dynamic arrays are pointers to heap blocks with size and reference count metadata stored immediately before the actual data:
  ...
  ```
* **Current state**: Implemented for scalar element types and, under `{$define PXX_MANAGED_STRING}`, `array of AnsiString` plus arrays of records recursively containing `AnsiString` fields. Nested arrays of those bases work at any depth. `var a: array of T;` declares a pointer-sized slot holding a heap data pointer; `SetLength(a, n)` allocates a `[refcount(8)][length(8)][elements...]` block and stores the data pointer into the slot; `Length(a)` reads the length word with a nil-guard (unallocated → 0); indexed read/write are 0-based. Assignment retains/releases shared storage. Indexed writes clone shared storage before mutation at every nesting level (copy-on-write), retaining sub-array handles and managed leaves on the clone. Resize preserves the retained prefix, zeroes new slots, and reclaims replaced blocks; `SetLength(a, 0)` releases storage. Local slots initialize to nil and release on normal scope exit. Managed element arrays recursively retain copied strings and finalize them when their final array owner is released. A dynamic-array `array of T` **field** of a record or class is a first-class managed slot at object+offset: `SetLength`, `Length`, indexed read/write, copy-on-write, retain-on-copy, and scope-exit finalization all operate on the field slot, with element metadata carried on the IR node (not a root symbol) for the field case. Class instances are zero-filled on construction so a freshly `Create`d managed field starts nil even when its backing block is reused from the free list. Params and function results are implemented. Refcount updates become atomic only under `--threadsafe`. Coverage: `test/test_dynarray.pas`, `test/test_dynarray_ansistring.pas`, `test/test_dynarray_managed_record.pas`, `test/test_dynarray_field.pas`, `test/test_dynarray_params.pas`, `test/test_dynarray_result.pas`, `test/test_nested_dynarray.pas`, `test/test_nested_dynarray_managed.pas`, `test/test_multithreading.pas`.
* **Remaining gaps**:
  - Exception-path cleanup.
  - (Done 2026-06-04: nested-level copy-on-write; fresh function-result move
    semantics; materialised argument-temp ownership.)
* **Ordering note**: The cross-thread policy is fixed: atomic refcount updates
  are emitted only in threaded builds. Atomic
