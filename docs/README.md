...
- [Handover: Managed-Strings Self-Compile — Record Layout (F2)](handover-managed-strings-record-layout-2026-06-05.md) - resume brief for the managed-string self-compile; gaps A/B/F1 done, only F2 (computed record-layout engine to replace hardcoded RecSize/RecFieldOffset) remains. Pairs with [Plan: Refcounted Compiler Strings](plan-refcounted-compiler-strings.md).
