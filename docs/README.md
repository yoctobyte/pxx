...
- [Handover: Managed-Strings Self-Compile — Stage-2 Growth](handover-managed-strings-record-layout-2026-06-05.md) - resume brief for the managed-string self-compile; gaps A/B/F1/F2 are done and gate-clean, the FPC-seeded managed compiler builds and runs `hello.pas`, and the remaining blocker is stage-2 managed self-compile CPU/RSS growth before fixedpoint. Pairs with [Plan: Refcounted Compiler Strings](plan-refcounted-compiler-strings.md).
