...
- Compiler/lib gap → ticket in `docs/progress/{urgent,backlog}/`, regen board
...
- `--smoke` validates **logic, not GTK render**. ALWAYS screenshot a GUI change:
  ...
  `GUI_SHOT_FRESH=1`. See `docs/developer/gui-testing.md`.) No xdotool → can't
...
## Epic + what's DONE
Epic: `docs/progress/{backlog,done}/feature-eliah-shell.md` — one window, splitter
...
## Start
Read `docs/progress/BOARD.md`, the three tickets above, and
