...
| C interoperability | Header imports drive a real library end-to-end (SQLite from `/usr/include/sqlite3.h`, linked `libsqlite3.so.0`): pointer/handle args, function-pointer params, automatic `string`→`const char*` marshalling (explicit `PChar()` only needed for non-pointer targets), object-like integer `#define` macros surfaced as named constants (e.g. `SQLITE_ROW`), `typedef struct { ... }` bodies laid out as real records (field access, nested structs, unions), and typed pointers carrying their element type (`T*` deref/index; pointer typedefs become typed Pascal pointers). C pointer depth is recorded for imported parameters so Nil Python can lift trailing `T**` out-params. Not a full C frontend or ABI surface. |
| Nil Python (`.npy`) | `import name` routes to the unit/C-header resolver; imports C headers and drives full SQLite CRUD directly through `sqlite3` with no required Pascal wrapper. Locals infer from a callee's return type, and returned C strings are copied into managed strings. |
