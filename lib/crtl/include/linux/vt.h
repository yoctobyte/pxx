...
/*
 * C runtime: <linux/vt.h> -- the virtual-console ioctls.
 *
 * struct vt_stat IS THREE __u16 AND THE FIRST ONE IS WHY VT_GETSTATE CANNOT
 * SEE PAST CONSOLE 15: v_state is a bitmask of active vts in SIXTEEN bits, and
 * the kernel says so in its own comment. A version that widened the fields
 * would compile, and would hand the kernel a struct whose second field is at
 * the wrong offset -- openvt would then switch to the console named by half of
 * v_signal.
 *
 * Found attempting busybox on i386: console-tools/openvt.c (VT_OPENQRY,
 * VT_GETSTATE, VT_DISALLOCATE) and init/init.c.
 * feature-c-corpus-busybox-i386-the-second-architecture
 */
#ifndef _CRTL_LINUX_VT_H
#define _CRTL_LINUX_VT_H
...
struct vt_stat {
  ...
  __u16 v_state;   /* vt bitmask -- SIXTEEN bits, see the note above */
};
