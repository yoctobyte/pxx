...
/*
 * C runtime: <linux/i2c.h> -- the I2C/SMBus transfer structs and capability bits.
 *
 * I2C_M_RD IS A FLAG IN struct i2c_msg AND I2C_SMBUS_READ IS AN ARGUMENT TO A
 * DIFFERENT IOCTL. They are both 1 today, in two vocabularies that have no
 * reason to stay equal: i2c_msg.flags is a BITMASK (I2C_M_TEN,
 * I2C_M_RECV_LEN and the rest sit alongside), while i2c_smbus_ioctl_data's
 * read_write is a plain 0/1. Writing one where the other belongs works until a
 * second flag is ORed in.
 *
 * union i2c_smbus_data ENDS IN A 34-BYTE ARRAY and the odd number is the
 * point: 32 bytes of block payload, plus a length byte, plus one spare, which
 * the kernel's own comment records. Sizing it 32 compiles, and lets the kernel
 * write two bytes past the end of the caller's union on a full block read.
 *
 * Found attempting busybox on i386: miscutils/i2c_tools.c.
 * feature-c-corpus-busybox-i386-the-second-architecture
 */
#ifndef _CRTL_LINUX_I2C_H
#define _CRTL_LINUX_I2C_H
