...
#ifndef PXX_CRTL_LINUX_WIRELESS_H
#define PXX_CRTL_LINUX_WIRELESS_H 1

/* Linux Wireless Extensions -- the SIOCxIWx ioctl numbers and their payloads.

   FOUND BY ATTEMPTING busybox FOR i386 at the 394-applet scope, and it is the
   FOURTH layer of one two-translation-unit peel: networking/ifplugd.c could not
   ask for this header until <linux/if.h>, <linux/mii.h> and <linux/ethtool.h>
   had each landed, because a translation unit reports ONE missing include and
   stops. x86-64 never saw any of the four -- the host-header fallback is
   native-only, so there an unknown <h> resolves from /usr/include with a
   warning and the TU compiles against GLIBC's copy.
   feature-c-corpus-busybox-i386-the-second-architecture

   THE LAYOUT IS THE INTERFACE, not the field names. `struct iwreq' is the
   ioctl payload: the driver writes into whatever the caller supplied, so a
   union member that is short does not fail at the ioctl -- it corrupts
   whatever the caller put beside it. busybox's ifplugd reads the associated
   AP's MAC out of `u.ap_addr.sa_data' after SIOCGIWAP, which is at a fixed
   offset inside a union sized by its LARGEST member; get `struct iw_point'
   wrong and that offset is still valid and still wrong.

   SO THE CONSTANTS AND THE STRUCT BODIES ARE EXTRACTED FROM THE HOST'S
   <linux/wireless.h> RATHER THAN TYPED. Every value here is a kernel or wire
   constant with nothing to derive it from, and the host's header is the only
   oracle; transcribing 233 constants and 19 aggregates by hand is a source of
   silent single-digit errors that no size assertion would catch.
   test/c_crtl_uapi_wireless.c diffs the whole file against `gcc' -- which
   reads /usr/include -- so it is a real differential and not a restatement of
   what we wrote, and it runs at BOTH widths, i386 included, because i386 is
   the target where no host-header fallback can make a wrong file look right.

   `struct iw_point' CARRIES A POINTER, so its size and every offset after it
   in `union iwreq_data' differ between 32- and 64-bit. That is the reason the
   union's size is asserted at BOTH widths rather than as one number.

   THE MACRO SET IS THE ORACLE'S, EXACTLY -- and that is a CORRECTION. This
   file first shipped without WIRELESS_EXT, the eleven IWEV* event codes,
   IW_EV_POINT_LEN and the IW_EVENT_CAPA_* / IW_IS_* helpers: 24 macros the
   extractor had dropped, defended afterwards as a decision not to carry
   "function-like macros nothing in the corpus invokes". Two of those three
   families are plain integer constants, so the sentence was a rationale
   written around an omission rather than for one.
   pxx does not refuse an undeclared identifier used as a value -- it warns and
   yields 0 -- so every missing constant COMPILED, and the only thing that
   noticed was the gcc diff. A header is now checked by CENSUS as well as by
   layout: test/c_crtl_uapi_wireless.c prints each macro's value, so one
   missing from here shows up as a 0 against gcc's number rather than as
   nothing at all. Keeping the set equal to the oracle's is what makes that
   census meaningful; a deliberately partial header cannot be checked this way,
   which is the real argument against trimming it. */
...
/* Wireless EVENTS. These are *NOT* ioctls -- they are the `cmd' field of a
   struct iw_event delivered over rtnetlink, and they share the 0x8Cxx range
   rather than the 0x8Bxx one above. */
...
/* The event_capa[] bitmap in struct iw_range, addressed by ioctl number.
   IW_EVENT_CAPA_K_0 and _K_1 are the events the KERNEL generates for every
   device, so a driver ORs them in rather than listing them. */
...
#endif /* PXX_CRTL_LINUX_WIRELESS_H */
