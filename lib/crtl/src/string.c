 ...
 * and DEFINED here for the same reason. It was in signal.c, where it reads
 * more naturally, and that made it unreachable: including only <string.h>
 * auto-pulls crtl/src/string.c and nothing else, so the symbol resolved from
 * the system libc instead. Caught by tools/crtl_reachability.py. The table is
 * indexed by signal number and names no SIG* constant, so living here costs
 * this file no new include.
 *
