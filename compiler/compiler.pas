...
function CSynthMissingHeaderFor(const soname: AnsiString; var header: AnsiString): Boolean; forward;   { real body beside the ld.so.cache reader in pasparser_proc.inc, which is included AFTER symtab.inc. RegisterExternal is the choke point where an external becomes a real DT_NEEDED and so is where a soname the compiler invented from a file name has to be refused — but the list it asks about is written during header IMPORT, three includes later. Forwarded rather than duplicating the five-line scan into symtab.inc: two lookups over one array is the second-path smell normalise-dont-special-case.md is about, and PXX prescans headers while FPC is single-pass, so getting this wrong self-hosts cleanly and is caught only by gate.sh's FPC seed canary }
function CSynthDirAnswersFor(const soname, sym: AnsiString; var better: AnsiString): Boolean; forward;   { same reason and same place as the line above — but this one ANSWERS rather than refuses. RegisterExternal knows the symbol; the header's directory names a candidate library; the candidate's own .dynsym says whether it defines that symbol. Forwarded because the two halves of the question are three includes apart, exactly as CSynthMissingHeaderFor's note describes }
