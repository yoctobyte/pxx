...
function PasCondNameDeclaredInUses(const nm: AnsiString): Boolean; forward;   { real body in pasparser_proc.inc beside ResolveUsesUnitSource, because answering `{$if declared(X)}` for a name in a USED unit means resolving that unit's source, and the search chain is one copy that lives with the parser. paslexer.inc asks the question; it holds no chain of its own, same reason as PasCondSizeOfTypeName above (bug-p-declared-cannot-see-a-used-units-declarations) }
