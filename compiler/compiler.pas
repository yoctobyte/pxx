...
procedure CMarkTokModuleId(startTok, modId: Integer); forward;   { ditto }
procedure CMarkTokNoModule(startTok: Integer); forward;          { ditto }
...
function PasCondOrdBoundOfTypeName(const nm: AnsiString; wantHigh: Boolean; var v: Int64): Boolean; forward;   { the VALUE twin of PasCondSizeOfTypeName below, and forwarded for the identical reason: it asks OrdinalNameToTk + OrdinalTypeBound, the pair the constant evaluator already uses, so no range table is ever written into paslexer.inc. `{$if declared(X) and (X<>RS_INVALID)}` with `RS_INVALID = high(tsuperregister)` (FPC cgbase.pas:400) is what needs it }
function PasCondSizeOfTypeName(const nm: AnsiString): Int64; forward;   { real body beside BuiltinTypeNameTk in pasparser_lval.inc, which is THE one name-to-kind table; paslexer.inc's `{$if sizeof(Extended) <> sizeof(Double)}` asks it, and paslexer.inc is included here. A width table written into paslexer.inc would be a SECOND SOURCE OF SIZE TRUTH, which is the defect BuiltinTypeNameTk's own comment records three separate fixes for -- so this forwards a QUESTION and carries no answer (ditto) }
function PasCondNameDeclaredInUses(const nm: AnsiString; arity: Integer): Boolean; forward;   { real body in pasparser_proc.inc beside ResolveUsesUnitSource, because answering `{$if declared(X)}` for a name in a USED unit means resolving that unit's source, and the search chain is one copy that lives with the parser. paslexer.inc asks the question; it holds no chain of its own, same reason as PasCondSizeOfTypeName above (bug-p-declared-cannot-see-a-used-units-declarations) }
function PasCondProbeUsedUnits(const nm: AnsiString; arity, question: Integer): Boolean; forward;   { the SAME walk as the line above and now its only body: it lexes each used unit and points a range predicate at the result, and there are three predicates -- declared / const value / type alias (PCQ_* in paslexer.inc). Generalised rather than copied because the walk is the expensive and subtle half (ResolveUsesUnitSource, the probe-depth guard against a legal `a uses b, b uses a`, the define/directive reset per unit) and a second copy of it would be the drift this file's other forwards each record; the answers a Boolean cannot carry come back in PasCondProbeInt/PasCondProbeStr, read immediately beside the call (bug-p-a-conditional-directive-cannot-read-a-constant-or-a-type-the-source-declares) }
function CSynthMissingHeaderFor(const soname: AnsiString; var header: AnsiString): Boolean; forward;   { real body beside the ld.so.cache reader in pasparser_proc.inc, which is included AFTER symtab.inc. ResolveSynthImportLibraries is the choke point where an external becomes a real DT_NEEDED and so is where a soname the compiler invented from a file name has to be refused — it runs from the three DT_NEEDED builders in elfwriter.inc, NOT from RegisterExternal, which is where it sat until 2026-09-10 and is one parse away from the last write to ProcLibrary (bug-c-an-unresolvable-synthesised-soname-still-reaches-dt-needed) — but the list it asks about is written during header IMPORT, three includes later. Forwarded rather than duplicating the five-line scan into symtab.inc: two lookups over one array is the second-path smell normalise-dont-special-case.md is about, and PXX prescans headers while FPC is single-pass, so getting this wrong self-hosts cleanly and is caught only by gate.sh's FPC seed canary }
function CSynthDirAnswersFor(const soname, sym: AnsiString; var better: AnsiString): Boolean; forward;   { same reason and same place as the line above — but this one ANSWERS rather than refuses. the caller knows the symbol; the header's directory names a candidate library; the candidate's own .dynsym says whether it defines that symbol. Forwarded because the two halves of the question are three includes apart, exactly as CSynthMissingHeaderFor's note describes }
function CSynthDirCandidateFor(const soname: AnsiString; var cand: AnsiString): Boolean; forward;   { diagnostic only: names the directory-derived library that was tried and did NOT answer, so the net/if.h shape reads as a diagnosis instead of a dead end }
function AIntToStr(n: Integer): AnsiString; forward;   { real body in util.inc, which is included AFTER lexer.inc because it needs AppendChar. paslexer.inc's declared() warning names how many units it could not read, and FPC is single-pass -- the seed canary caught this one, which is the whole reason it exists }
procedure RetagSetLitAsArrayCtor(node: Integer; elemTk: TTypeKind; depth: Integer); forward;   { real body in ir.inc, beside the AN_ASSIGN arm that is its other caller; pasparser_expr.inc's `TDynArr.Create(...)` arm calls it too, so the DELPHI CONSTRUCTOR SPELLING AND THE LITERAL SPELLING RETAG THROUGH THE SAME CODE. A second copy in the parser is the shape normalise-dont-special-case names: the two would agree today and the nested-row recursion, the AN_ARRAY_CTOR depth encoding in ASTSOffset and the elemTk write would then have to be kept in step by hand (ditto) }
function SetLitElemsArePlain(node: Integer): Boolean; forward;   { real body in ir.inc beside RetagSetLitAsArrayCtor above, because the two are asked TOGETHER at every door -- may this set literal be retagged, and then retag it. pasparser_expr.inc's concatenation gate (DynConcatOperandsEligible) is the third caller. Caught by gate.sh quick's FPC SEED CANARY and by nothing else: pxx prescans headers so `make compiler/pascal26` and --tier quick both passed, while FPC is single-pass and had not seen the declaration -- the exact defect class CLAUDE.md says the canary is the only instrument for (ditto) }
function IsVarArgLvalueCast(n: Integer): Boolean; forward;   { real body in pasparser_call.inc; symtab.inc's CheckVarArgWidth asks it so an explicit `P(Int64(a))` keeps the cast's width (ditto) }
function TakeStrInitSpan(var sOff, sLen: Integer): Boolean; forward;   { real body in pasparser_decl.inc. Forward-declared because _expr.inc is included FIRST (188) and _decl.inc second (190), and TryParseInitValForm -- in _expr.inc -- is the one door every initialiser form goes through, so it must be able to ask the same question the _decl.inc callers ask. The alternative was a second copy of the FindStrConst lookup inside the helper, which is the shape normalise-dont-special-case names: two spellings of "take a string initialiser span" that agree today and drift the first time one of them learns something (the one-character-const arm is exactly such a thing, learned once already) }
function NodeProcSlotSig(node: Integer): Integer; forward;   { real body in ir.inc, where it is read by the procedural-slot store guard. pyparser.inc's NilPy assignment builder asks the SAME question for the SAME reason -- is this lvalue a procedural slot -- across all three shapes (symbol, array element, field), so a second copy would be a second answer to one question. FIFTH forward on this exact seam, every one a parser file reaching into ir.inc; pxx prescans headers so `make compiler/pascal26` and --tier quick both pass with it missing, and gate.sh quick's FPC SEED CANARY is the only instrument that sees it (ditto) }
function NodeDynElemRowLen(node: Integer): Integer; forward;   { real body in ir.inc beside the other two above, and asked in the same breath as them: pasparser_stmt.inc's Insert arm must know whether the destination's ELEMENT is a fixed ROW before it retags an element list, because a row is bytes inside the outer block rather than a handle and the array constructor has nothing to build for it (the limit tarray15 records). THIRD forward added here today for the same reason and by the same instrument -- every one was a parser file reaching into ir.inc, and pxx prescans headers so `make compiler/pascal26` and --tier quick pass all three (ditto) }
function FixedArrayAsArrayCtor(srcNode, rowLen, rowLo: Integer): Integer; forward;   { real body in ir.inc beside the three above, and it is the same door: `d := s` and `Insert(s, d, i)` both need a FIXED-length array turned into the element list the array constructor already knows how to materialise, because a fixed array is bare bytes with no [len:8] header and the bare store put its ADDRESS in the handle slot. pasparser_stmt.inc's Insert arm is the parser-side caller. FOURTH forward on this seam -- every one a parser file reaching into ir.inc, and pxx prescans headers so the canary is again the only instrument that sees it (ditto) }
function StaticArraySourceInfo(argAST: Integer; var cnt: Integer; var elemTk: TTypeKind; var elemRec: Integer; var elemSz: Integer): Boolean; forward;   { real body in ir.inc. pasparser_stmt.inc's Finalize desugar and Dispose must know whether the argument is a WHOLE FIXED ARRAY: the node's type tag is the ELEMENT's, so without asking, `Finalize(arrOfStr)` desugared to `arr := ''` and `Finalize(arrOfRec)` finalized element 0 only. }
{$include lexer.inc}
...
{$include elfdynsym.inc}   { the ELF .dynsym reader — BEFORE pasparser_proc.inc, whose LdCacheDirCandidate calls ElfIsNative64/ElfLoadMemo to pick a candidate library of this machine's own architecture. Its own file so test/test_elfdynsym.pas can include it standalone and assert the refusal arm, which no header on this box can reach }
