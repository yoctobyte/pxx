...
function FixedArrayAsArrayCtor(srcNode: Integer): Integer; forward;   { real body in ir.inc beside the three above, and it is the same door: `d := s` and `Insert(s, d, i)` both need a FIXED-length array turned into the element list the array constructor already knows how to materialise, because a fixed array is bare bytes with no [len:8] header and the bare store put its ADDRESS in the handle slot. pasparser_stmt.inc's Insert arm is the parser-side caller. FOURTH forward on this seam -- every one a parser file reaching into ir.inc, and pxx prescans headers so the canary is again the only instrument that sees it (ditto) }
{$include lexer.inc}
