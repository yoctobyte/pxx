...
lib-test: pxx-stable-check
	...
	test "$$(/tmp/lib_http | grep -c '=ok')" = "46"
	...
	test "$$(/tmp/lib_classes | grep -c '=ok')" = "17"
	...
	@echo "lib-test ok (sudoku exact + collections + math + sysutils + random + bitset + platform + directory + bignum + json + calc + sat + mathf + vm + mandelbrot + lisp + zlib + png smoke + ansiterm + ansirender + process + process-multi + dynlibs + unixshims + strpchar + sockets + http + http-async + http-redirect + http-keepalive + http-pool + dns-async + classes + strutil) against stable v$$(cat $(STABLE_DEFAULT_DIR)/VERSION 2>/dev/null || echo '?')"
