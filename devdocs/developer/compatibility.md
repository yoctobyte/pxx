...
| Broader Object Pascal model | Exact user-class exception handlers, finalizers, re-raise, properties, class inheritance, explicit `inherited` calls, virtual dispatch, published RTTI, named `class of` metaclass aliases, record/set aggregate returns, and CORBA-style interfaces (decl/implement/call, is/as/Supports, implicit coercion, identity, inheritance — all four targets) are covered. Interface automatic reference counting (COM-style ARC) is deferred; strict descendant enforcement for pointer-backed metaclass aliases, exception hierarchy matching, and rich exception messages remain missing or incomplete. |
