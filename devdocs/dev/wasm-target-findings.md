...
Measured 2026-08-27 against `dev@7c03abff3`, during the scoping session that
opened `feature-target-wasm` and
`refactor-a-target-properties-have-no-single-answer`.
...
`elfwriter.inc:1926` — *"Method-pointer fixups: patch the 32-bit code address
...
36 files reference `TargetArch`, in raw `if` / `else if` chains with an implicit
x86-64 fallthrough, and **no `TargetPtrSize` / `TargetIs32` helper exists**.
A 7th target — wasm32, i.e. 32-bit pointers — is exactly the case those chains
answer wrongly and silently.

This is filed separately and is a prerequisite:
`devdocs/progress/urgent/refactor-a-target-properties-have-no-single-answer.md`.
It is worth doing on its own merits; wasm is the forcing function, not the
justification.
...
| --- | --- | --- | --- |
| `ir_codegen_wasm32.inc` | A (new file) | ~3-4k | `ir_codegen_riscv32.inc` = 3,655 |
| `wasmenc.inc` + module writer | A (new file) | ~1-1.5k | `elfwriter.inc` = 3,729; wasm sections are far simpler |
...
| `TargetPtrSize` refactor | **A (shared)** | — | filed separately, prerequisite |
