# Handbook rationale — the long form of CLAUDE.md's rules

Archived 2026-08-31, when CLAUDE.md was cut from 72KB to a rules-only file.
Owner: *"what should be 10k of instructions became 300k. That's the part that
is unacceptable and is wasting tokens on any new session."*

**Nothing here is an instruction.** CLAUDE.md is the authority; this is the
evidence, the incidents and the reasoning behind its rules, kept because most
of it was measured and would otherwise be re-derived badly. Look up what you
need; do not read the file.
