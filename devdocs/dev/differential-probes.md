...
| `tools/scalar_cast_door_probe.py` | FPC 3.2.2 **+ a route twin check** | for a named-type SCALAR cast: does every type name answer the same through the recognition rule that reaches it directly AND through a user alias declared to it, and do both match FPC? 24 names x 3 truncating values x 2 routes. The axis is the ROUTE, because every door in this family is correct for the values its own tests used -- **exits 1** if a name answers differently through the two routes, or if the must-differ control agrees | P |
