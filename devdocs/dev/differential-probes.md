...
| `tools/scalar_cast_door_probe.py` | FPC 3.2.2 **+ a route twin check** | for a named-type cast: does every type answer the same through the rule that reaches its name directly AND through a user alias declared to it, and do both match FPC? TWO families. (a) 29 integer/char/boolean NAMES x 3 truncating values, stored in the declared type. (b) 10 CATEGORY rows added 2026-09-06 after family (a) ran green through six live defects: float / variant / enum / record-with-`operator Explicit` targets, variant and record OPERANDS, the result passed to a procedure rather than STORED (storing coerces and hides a reinterpret), and `{$PACKENUM 1}` as a file-scoped axis. A row whose defect is a lost IDENTITY must print INLINE -- a typed parameter rebuilds the identity and the row passes on a broken binary. The axis is the ROUTE in both, because every door here is correct for the values its own tests used -- **exits 1** if a name or a category row answers differently through the two routes, or if the must-differ control agrees | P |
| `tools/call_result_suffix_probe.py` | FPC 3.2.2 **+ a route twin check + a must-differ control** | for the postfix `^ / .field / [i]` chain: does it answer the same off a CALL RESULT as off a pointer VARIABLE holding the same address, and do both match FPC? The axis is the ROUTE, because both of the two loops that parse this chain are correct for the shapes their own tests use. Rows are chosen from a HELPER CENSUS of the two loops rather than from imagination — every helper the call-result loop never calls gets a row. **Exits 1** if a route pair disagrees, if the must-differ control comes back equal, or if either compiler failed to build-and-run the probe | P |
| `tools/type_name_every_door_probe.py` | FPC 3.2.2 **+ two branched controls** | for ONE type name, does EVERY door agree with FPC: the declaration, `SizeOf(T)`, a cast STORED IN ITS DECLARED TYPE, the cast's value, `High`, `Low`, `TypeInfo`. Names are read out of `pasparser_lval.inc` (the UNION of `OrdinalNameToTk` and `BuiltinScalarTypeKind`), so a name added to either table is swept without editing the probe. The finding it exists for is a name ACCEPTED at some doors and REFUSED at others while fpc accepts everywhere — the `SizeUInt` shape, invisible to a per-door test because every working door says the name is fine, and hidden further when the name has synonyms that work. **PHASE 2, no oracle:** every name is then asked again through a one-level alias (`type zz_alias = ByteBool; High(zz_alias)`) at `SizeOf`/`High`/`Low`/`TypeInfo`, and the two SPELLINGS are compared against each other inside pxx. That is a different RECOGNITION path — `FindTypeAlias` answers instead of the builtin-name table, at a different site — and the direct spelling is the one every test in the tree is written in, so an arm reachable only through an alias has never been asserted on. It found `High`/`Low` of ByteBool/WordBool/LongBool REFUSED direct and ANSWERING 255/65535/2147483647 through an alias, where the refusal is the deliberate mitigation; and `Comp`, which no hand-picked row set had included because it reads as a float. Self-consistency is the verdict available when a second implementation is not, and here it is also the RIGHT one: two spellings of one name disagreeing needs no oracle. Carries its own must-differ control (`SizeOf(byte)` against `SizeOf(alias of int64)`), because a phase whose healthy output is silence prints exactly that when it is broken. **Exits 1** if `zzznosuchtype` is accepted at any door, if `integer` is refused at any, if a spelling disagrees, or if the COMPILER MOVED under the sweep — it hashes the binary before and after, because a mid-run rebuild partitions the results along the ALPHABET and reads as a real per-name difference rather than as noise | P |
