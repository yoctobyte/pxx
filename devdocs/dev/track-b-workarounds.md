...
| `lib/rtl/mimic_urllib_parse.py` (`SplitResult.__getitem__`, `ParseResult.__getitem__`) | the field tuple bound to a local before being indexed, instead of `return (self.a, self.b)[i]` | [[bug-n-subscripting-a-container-literal-inside-a-function-segfaults]] — subscripting a container literal inside a function body segfaults; module level and a bound local are both fine | index the literal directly |
