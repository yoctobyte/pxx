...
| `lib/rtl/dns_cache.pas` (`TDnsCache`) | slot array declared as a named type `TDnsCacheSlots = array[0..DNS_CACHE_SLOTS-1] of TDnsCacheEntry` instead of an **inline** `array[...] of TDnsCacheEntry` record field | [[bug-inline-array-field-const-bound]] — inline record-field array with a const-expr bound mis-parses | inline the array in the `TDnsCache` field (though the named alias is arguably cleaner — low-priority revert) |
| `lib/rtl/dns_cached.pas` (whole unit) + `lib/rtl/dns_async.pas` | the cache-consuming resolver lives in a **separate** `dns_cached` unit; `dns_async` (hence `http`) deliberately does **not** `uses dns_cache` | [[bug-transitive-dns_cache-import-corrupts-managed-strings]] — transitive `uses dns_cache` corrupts http's managed-string codegen → segfault | fold `DnsQueryAListCachedAsync` back into `dns_async` and let the facades (`DnsResolveHostAsync`, blocking `DnsResolveHost`) import `dns_cache` directly |
