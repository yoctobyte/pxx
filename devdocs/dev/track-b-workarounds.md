...
> bottom section.
>
> **A third state exists and neither section names it: fixed at HEAD, not yet
> pinned.** Track B builds with `$(PXX_STABLE)`, so a fix can be landed,
> resolved and in `done/` while still unusable here — and then the file's own
> re-check instruction ("verify the bug ticket is still in `backlog/`/`blocked/`
> before assuming the workaround is still needed") gives the WRONG answer: the
> ticket is closed, so the row looks revertible, and reverting it breaks the
> lane's gate. For these rows **the blocker is the pin and the check is a
...
Four rows. The first two re-verified **by behaviour** at pin v393
