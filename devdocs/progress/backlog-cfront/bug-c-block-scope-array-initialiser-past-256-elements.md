...
| --- | --- | --- |
| `nArrElems > 255 then Error(...)` | general expression element | REFUSES, loudly |
| two `nArrElems <= 255` | string literal standing for a char ROW | copies part of the row, leaves the tail zero, says nothing |
| two `nArrElems := -1` | the two skip arms | drops the whole initialiser, says nothing |

The loud one is a refusal and not a wrong answer, so it is the least urgent
part. The three silent ones are the same failure mode the file-scope bug had:
a plausible wrong value with no diagnostic.
