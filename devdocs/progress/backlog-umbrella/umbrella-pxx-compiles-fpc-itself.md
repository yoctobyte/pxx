...
Of the 17 units that are neither TDoubleRec nor the object wall, **12 are RTL
gaps, not frontend walls**: PUnicodeChar + `cp1251` (7), `swapendian` (1),
`entryreal_bytes` + swapendian (4 more count them together), `unixcp` (1),
`heaptrc` (1). Per this file's own WALL 5 note, **an RTL wall is LIVE without a
pin and a compiler wall is not** — so if anyone wants delivered units rather
than cleared walls, that is where the cheap ones are.
