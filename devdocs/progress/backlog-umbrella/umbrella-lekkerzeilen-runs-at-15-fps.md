...
### THE PHRASE `quiet box` IN THIS FILE IS AN ASSERTION, NOT A RECORDED CONDITION — added 2026-09-22

It appears in several places above and below, including in the table row
immediately overhead, and **not one of them has a load figure behind it.** No
absolute in this file was ever written down with the machine's state beside it;
the phrase was supplied by the seat taking the measurement, from the fact that it
had not started anything else.

`frankuser` measured this workstation on 2026-09-22, after the owner mentioned
that an unrelated project of his is using its GPU, storage and CPU: **load
average 23.93, GPU 4195 of 6144 MiB, `/` at 95% with 8.3G free** — and the load
figure was relayed as *"on a 24-thread box"* when this box has **12** (one Xeon
E5-2620 v2, 6 cores; `nproc`, `nproc --all` and `/proc/cpuinfo` all agree), so it
means about **twice oversubscribed**, not about saturated. A load average is a
ratio with its denominator left off, which makes it the easiest figure here to
relay wrong. `/` is the volume the repo sits on; `/tmp` is separate, at 6% with
1% of inodes used. Whether that load was present during the roofs run is **unknown and
unrecoverable** — which is the whole point. An assumption is withdrawn here; a
measurement is not refuted. The absolutes are not shown to be wrong.

**What survives by construction:** every DELTA whose arms were interleaved inside
one session — the 3%, the 19.7x, the A/B rows — because a common-mode load enters
both arms and cancels in the difference. **What does not:** every LEVEL.

**Why this belongs in the file and not in a message.** The rule one paragraph
down — *a reachability factor is a property of the scene AND the machine state it
was measured on* — is this ticket's own, is correct, and was **violated by this
ticket in the direction the rule exists to catch**, several times, by stating the
machine state in prose rather than recording it. A condition asserted in prose
cannot be re-derived once the condition is gone, so it is the one claim a later
reader has no instrument for. **Print the load average beside any future
absolute** — it costs a word and it is the only thing that makes a level
quotable.

**No occurrence COUNT is given here, on purpose, and the reason is worth the
sentence.** This section's own heading spells the phrase — deliberately, so the
section is findable by the same `grep` that finds the problem — which means this
note has joined the population it is about. A number written here would be wrong
for every reader but its author, and a guard that is born red teaches people to
ignore it. `grep` for it and read the hits; the criterion is *does any hit have a
load figure beside it*, which is a question about the hits and not about their
number.
