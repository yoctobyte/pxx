...
## 2026-09-12 summary, superseded

Kept verbatim because the `with ... as` entry is a diagnosis lesson, not a
changelog row, and because this umbrella's summary is read far more often than
its body.

> 2026-09-12, LATEST — ALL OF app.py NOW COMPILES (4344 lines of warnings, no error). CURRENT WALL is `print(__doc__.strip())` in `__main__.py`'s `--help` path — `undefined variable (__doc__)` — and CITE THE CONSTRUCT, NOT THE LINE: that file is modified on disk by an in-flight backend port, so it is line 312 in the owner's working tree and 299 at HEAD. The gap is real in both (each opens with the same leading docstring); only the citation moves — the module docstring dunder, read in the `--help` path as `print(__doc__.strip())`. UNASSESSED: nobody has yet sized whether NilPy should bind a module's leading string literal to `__doc__`, so do not read its position here as a decision that it should. READ THE LINE NUMBER CAREFULLY — it prints as a bare `pascal26:312:` with NO file name, the documented trap, and grep settled it: `lekkerzeilen/__main__.py:312` is the only `__doc__` inside the package (the nine others are in `tools/*.py`, which the closure does not reach). TEN WALLS CLEARED 2026-09-12, closure 1240 -> `__main__.py:312`. THE CHARACTER OF WHAT IS LEFT HAS CHANGED AND IT IS NOW MEASURED RATHER THAN PREDICTED: the first seven walls were parser gaps, and three of the last four were LIBRARY gaps — `tempfile.mkdtemp` (115 -> 138), `os.rmdir` beside it, and `zlib.ZLIB_VERSION` (141 -> 312). A seat picking this up should expect lib/rtl and builtin work, not parser work. TWO OF THOSE THREE ARE INERT FOR `$(PXX_STABLE)` UNTIL THE OWNER PINS, because `os.rmdir` is a BUILTIN addition (`compiler/builtin/pylib.pas` + `pypal.pas`); `mkdtemp` and `ZLIB_VERSION` are lib/rtl and live immediately. `make pin` is owner-only and nobody is waiting on one. ZLIB_VERSION WAS A VALUE DECISION, NOT A GAP, and it is the one thing here worth overruling cheaply: there is no libz under `lib/rtl/zlib.pas` (RFC 1950/1951 from scratch), so it reports `pxx-rtl` rather than a libz release number — the caller prints it to say WHICH zlib produced its bytes, directly above its own note that a differing `file` column is not a fault, so a spoofed version would defeat the line's only purpose. The cost is real and stated in the unit: a program doing version arithmetic on it raises where CPython returns numbers. The earlier narrative of this summary — including the `with ... as` diagnosis lesson, which is the one most worth reading — moved to the body section "2026-09-12 summary, superseded" rather than being dropped.

> 2026-09-12, LATEST — ALL OF app.py NOW COMPILES. CURRENT WALL `__main__.py:115`, `no member mkdtemp came of the qualifier tempfile` — blocker feature-n-tempfile-has-no-mkdtemp (track B, p70), and it is the FIRST wall in this closure that is a LIBRARY gap rather than a parser gap. All seven cleared before it were parser gaps, so the prediction this summary has been repeating ("what remains is parser gaps on constructs ordinary Python writes") has just expired — say so rather than carrying it forward. READ THE LINE NUMBER CAREFULLY: it prints as a bare `pascal26:115:` with NO file name, which is the documented trap — an error inside an imported module names no file and the reader supplies the one they invoked. Here grep settled it (the tree has exactly one `mkdtemp`) and it IS __main__.py, but do not read the next such line as app.py without checking. SEVEN WALLS CLEARED 2026-09-12, closure 1240 -> __main__.py:115, app.py fully past: extended-slice assignment (e3dcd6a35), a bare module attribute as a whole argument to a method (003154db3), chained assignment to nested attribute targets (203919198, corrected in b1215bf2e), a chain store through a VARIANT-typed receiver (da460243c), a float literal in a lambda body together with `{`/`}` (df8c96140), and `with C(...) as NAME` where NAME names a class. THAT LAST ONE IS THE ONE TO LEARN FROM AND THE LESSON IS ABOUT DIAGNOSIS, NOT ABOUT `with`. The cause was not in the `with` parser at all: `is` and `as` share one arm in the SHARED PASCAL expression parser, `is` carried a `not PyExprMode` guard and `as` did not, and FindUClass is case-INSENSITIVE — so `as window` resolved to `class Window` and that arm ate it as a CAST. Two failure modes from one cause, and the loud one was not the one that mattered: a NEW target name gave `undefined variable`, an ALREADY-BOUND one COMPILED AND KEPT THE OLD VALUE with no diagnostic. The ticket records TWO of my own diagnostic errors because both would mislead the next reader: its "REFUTED" list named the correct cause and dismissed it from a grep scoped to pyparser.inc alone (the NilPy parser delegates its arithmetic atom to the shared ParseExpr, which is what builds the node — so the grep was honest about the file and was read as an answer about the compiler), and the headline "__enter__ returns self" discriminator came from a four-row table that moved TWO VARIABLES, since every passing row also happened to use a non-colliding name pair. A one-variable ladder built from the WORKING program found it in three steps. ALSO FIXED, and it is a TEST defect rather than a compiler one: test_nilpy_the_threading_module asserted the order of two lines that were genuinely racing — `gate.set()` released the waiting thread BEFORE main printed — and it flipped once in a suite run sharing the box with a second compile. Thirty standalone runs at load 2.3 would not reproduce it, which is why it survived: it only flips when somebody else is measuring. `set()` now comes after the print, so the order is deterministic by construction and the row tests MORE than before. SUPERSEDED LEAD, 2026-09-12 — WALL WAS app.py:4237, `undefined variable (window)` on `with platform.open_window(...) as window:`. Blocker is bug-n-a-with-as-target-is-not-bound-when-enter-returns-self (p85), and it is PRE-EXISTING — identical on the PINNED compiler, so it is not a regression from tonight; it was simply hidden behind the walls in front of it. READ THAT TICKET BEFORE TOUCHING IT: the `as` target goes unbound only when __enter__ returns SELF, which is the idiom nearly every context manager uses, and the arm that matters is not the refusal. With the name PRE-DECLARED the program COMPILES AND KEEPS THE OLD VALUE (prints 1 where CPython prints 3), so a fixture asserting only the `undefined variable` row would pass the moment the symbol gets allocated whether or not the assignment happens. Narrowed, not solved: PXXDBG=a.ast shows __enter__ emitted as a DISCARDED statement, i.e. PyParseWithTail took its no-`as` arm, so the cursor was not on `as` when the check ran; PyCallMeth1 is the only candidate between the expression parse and the check and that is a HYPOTHESIS, not a measurement. Two theories are recorded as REFUTED on the ticket (the Pascal `as` cast — the kind=57 node in the dump is an IR coercion, not an AN_AS_CAST from the NilPy parser; and a syntactic recognition of `return self` — a class with no __enter__ at all fails the same way). SIX WALLS CLEARED 2026-09-12 and the closure moved 1240 -> 4237 (+2997 lines): extended-slice assignment (e3dcd6a35), a bare module attribute as a whole argument to a method (003154db3), chained assignment to nested attribute targets (203919198, whose own message misquoted the closure as 3305 — corrected in b1215bf2e), a chain store through a VARIANT-typed receiver (da460243c), and a FLOAT LITERAL in a lambda body together with `{`/`}` in one. Every one was a PARSER gap on a construct ordinary Python writes, not a missing library, which is the useful prediction for what remains. TWO LESSONS WORTH MORE THAN THE LINE COUNT. (1) The variant receiver needed NO new machinery — the statement path had solved it long ago (an AN_CLASS_CAST over AN_CALL(pyvarobj), plus a run-time class dispatcher for fields at differing offsets) and the chain path simply never asked; grep before proposing a mechanism, and read the prose when the grep returns prose. (2) The float wall looked like three other things: app.py:3305 is `lambda x, z: canopy.at(x, z, outside=ceiling + 1.0) <= ceiling`, so the keyword argument, the comparison and the method call were all better suspects, and `lambda x: 2.0` alone turned out to be enough. Every INT spelling compiled all along, which is the expected-value collision that makes an ordinary suite CERTIFY the gap rather than catch it. RETIRED HAZARD BLOCK, and it is worth knowing why it was right then and wrong now: this summary said in capitals that reaching 3305 by disabling the variant guard was NOT progress, because the same shape then stored NOTHING (CPython `11 11` against our `11 True`). That was measured and correct. The 3305 that followed was a DIFFERENT 3305 — stores verified row by row against CPython, the ambiguous-layout case included. The block also prescribed "FIX THE RECEIVER'S TYPE, NOT THE STORE", and that was wrong: the receiver does get a class identity, but via the cast the STORE builder places. A hazard block decays silently in the direction of doing nothing — re-measure one before obeying it. SUPERSEDED LEAD, 2026-09-12 — WALL WAS app.py:3305, a float literal in a lambda body, now fixed. FOUR WALLS CLEARED TODAY and the closure moved 1240 -> 3305 (+2065 lines): extended-slice assignment (e3dcd6a35), a bare module attribute as a whole argument to a method (003154db3), chained assignment to nested attribute targets (203919198, whose own message misquoted the closure as 3305 — corrected in b1215bf2e), and a chain store through a VARIANT-typed receiver. All four were PARSER gaps on constructs ordinary Python writes, not missing libraries, which is the useful prediction for what remains — and the fourth says something sharper: THE FIX WAS NOT NEW MACHINERY, IT WAS ASKING A DOOR THAT ALREADY EXISTED. The statement path had solved the variant receiver long ago (an AN_CLASS_CAST over AN_CALL(pyvarobj), plus a run-time class dispatcher for fields at differing offsets); the chain path simply never asked. Before proposing a mechanism for a NilPy wall, grep for it — and when the grep returns prose, read the prose. RETIRED HAZARD BLOCK, and it is worth knowing why it was right then and wrong now: this summary said in capitals that reaching 3305 by disabling the guard was NOT progress, because the same shape then stored NOTHING (CPython `11 11` against our `11 True`). That was measured and correct. The current 3305 is a DIFFERENT 3305 — the stores are correct, verified row by row against CPython including the ambiguous-layout case app.py actually needs. The block also prescribed "FIX THE RECEIVER'S TYPE, NOT THE STORE", and that prescription was wrong: the receiver does get a class identity, but via the cast the STORE builder places, so the work was in the store path after all. A hazard block decays silently in the direction of doing nothing — re-measure one before obeying it. SUPERSEDED LEAD, 2026-09-12 — CURRENT WALL app.py:3204, AND IT IS OUR OWN GUARD, DELIBERATELY: `in a chained assignment, the receiver of `visible` has no static class here`. Blocker is bug-n-a-chained-assignment-through-a-variant-typed-intermediate (raised 40 -> 85). DISABLING THAT GUARD REACHES 3305 AND IS NOT PROGRESS — measured, the same shape then compiles and stores NOTHING into the variant-receiver target (CPython `11 11` against our `11 True`), so those 101 lines are bought with a silent wrong store, which is this umbrella's own recorded trap (the closure compiling is not the demo running). DO NOT "unblock" the closure by relaxing it. MECHANISM NARROWED 2026-09-12 and TWO EARLIER HYPOTHESES REFUTED BY MEASUREMENT: the dynamic setter is NOT dropped by the chain's sequence and the receiver is NOT copied by PyForceVariant — a chain storing to an UNDECLARED attribute through the SAME variant receiver gives CPython's answer exactly (`22 22`). The fault is a DECLARED field reached through a variant receiver: the chain's store goes dynamic while the read of a declared field does not, so they disagree and the read returns the initial value. The SINGLE-statement spelling of the same store is correct, so the difference is the RECEIVER NODE'S TYPE, not the store builder — PyMakeAttrLoad types `self.menu` from UFldTk, which is not a class, where the expression parser's own receiver for the same source is. FIX THE RECEIVER'S TYPE, NOT THE STORE. And narrowing the guard to allow the fresh-attribute case would NOT unblock app.py, whose field (`visible`) is declared — do not spend the attempt. THREE WALLS CLEARED 2026-09-12 and the closure moved 1240 -> 3204 (+1964 lines): extended-slice assignment (e3dcd6a35), a bare module attribute as a whole argument to a method (003154db3), chained assignment to nested attribute targets (203919198, whose own message misquoted the closure as 3305 — corrected in b1215bf2e). Every one was a PARSER gap on a construct ordinary Python writes, not a missing library, which is the useful prediction for what remains. SUPERSEDED LEAD, 2026-09-12: THREE WALLS CLEARED TONIGHT AND THE CLOSURE HAS MOVED 1719 LINES (app.py:1240 -> 1678 -> 2360 -> 2959). CURRENT WALL: app.py:2959, `undefined variable (ui)` on `leading=ui.ROW` -- filed as bug-n-a-bare-module-attribute-is-undefined-as-a-whole-argument-to-a-method (p75) with an 11-line repro. IT IS NOT THE KEYWORD-ARGUMENT BUG IT LOOKS LIKE: the POSITIONAL form `self.m(1, ui.ROW)` fails identically and `b=1 + ui.ROW` compiles, so the discriminator is a bare `mod.ATTR` occupying a WHOLE argument at a method/constructor site, which the plain-function path handles correctly -- the same argument-position speculative-parse family as ad7c03b03. THE EXTENDED-SLICE WALL IS CLEARED and the lesson is about the FIXTURE, not the feature: PyRewriteSliceAssign's refusal was only reachable for a STATICALLY typed target, because the slice-assign-target predicate named pylist_slice_step and pybytes_slice_step and NOT pyvar_slice_step, so a target with no static type fell out of the store path entirely and died on `expected newline after statement` -- and that is any name first bound inside an `if` or `try` body. All eight top-level spellings compiled; only the fixture's try/except rows, which rebind their target inside the block, exposed it. A fixture written the ordinary way would have certified two thirds of the feature as working. The diagnostic also misreported the LINE, naming the first top-level occurrence of a similar statement 46 lines earlier. SUPERSEDED LEAD, 2026-09-12: TWO MORE WALLS CLEARED AND THE CLOSURE HAS MOVED 1120 LINES TONIGHT (app.py:1240 -> 1678 -> 2360). CURRENT WALL: app.py:2360, `assigning to an extended slice (with a step) is not implemented` -- an ALREADY-FILED ticket, bug-nilpy-an-extended-slice-cannot-be-assigned, found by a pydiff sweep on 2026-08-16 and sitting at prio 30; it is now wired here and inherits effective_prio, which is the ranker working as designed. The diagnostic is honest and precise, so it is a missing FEATURE, not a misresolution. The pop wall before it (`TPyDeque.pop() takes exactly 0 argument(s), got 2` on a dict field) was NOT the scan-order bug its ticket predicted: arFits and the arity promotion beside it asked the same question with different tests, so an overload-only match suppressed the run-time deferral and promoted nothing. SEVEN reductions of it failed and no regression fixture exists -- the closest passes pre-fix too, so committing it would be a guard that cannot fail; verification is this closure. One live bug found while reducing, filed separately: `collections.deque()` compiles and SEGFAULTS (p70). SUPERSEDED LEAD, earlier 2026-09-12: THE STAR-FOLLOWER WALL IS CLEARED AND THE CLOSURE MOVED 438 LINES FURTHER INTO app.py. A KEYWORD argument after a `*` unpack now works at all three METHOD sites; the fix was not the index-split the p75 ticket predicted but a CAP -- the compile-time expander was greedy (`wanted := ParamCount - firstSlot`, every remaining slot), and a keyword names its own slot, so stopping the star at the lowest slot a trailing `name=` claims is the whole change. CURRENT WALL: `tile.grids.pop(name, None)` refused with `pop() takes exactly 0 argument(s), got 2` -- a dict field resolved against a list's or deque's overload set, filed as bug-n-a-dict-field-resolves-pop-against-a-list-or-deque-overload-set (p80). FOUR REDUCTIONS OF IT FAILED; the ticket lists them so nobody repeats them, and says the next step is an INSTRUMENT (the arity error does not name the receiver class) rather than a fifth reduction. Still refused and separately ticketed: a trailing POSITIONAL after a star (needs the run-time length), and the CONSTRUCTOR arm of the keyword fix (its loop resolves a keyword to a FIELD index, not a parameter slot). SUPERSEDED LEAD, 2026-09-11: WALL NOW FULLY DIAGNOSED AND FILED as feature-n-a-method-call-cannot-take-an-argument-after-a-star-unpack (p75): the SAME star-plus-trailing-argument shapes work on a free function and are refused on a method, because plain calls route to the run-time forwarder and the three method sites still use the compile-time expansion that claims every remaining slot. LATE 2026-09-11: the closure now fails on `an argument after *unpacking is not supported yet` in app.py -- a KEYWORD argument after a `*` unpack, `f(*self.start[:2], forced=...)`, a shape feature-n-a-call-cannot-unpack-a-sequence-into-its-arguments did not cover and which is now the only thing between this umbrella and a compiling entry point, as far as a first-failure instrument can see. FOUR WALLS CLEARED IN ONE EVENING: ctypes in capture.py (corpus seam rewrite, uncommitted in /home/neo/lekkerzeilen); `world.label(...)` (55981bc63, f004e408e -- a unit-qualified member named like a Pascal reserved word); `import threading` (no code, just the --threadsafe flag its own diagnostic prescribes); and the type-widening refusal in App.open_region, which was NOT the conditional it appeared to be but `world.open(...)` being typed by fcntl.h's C `int open(...)` instead of world.py's own -- found only after the diagnostic was taught to print tyInt32 instead of 11. Still REQUIRES --threadsafe -dSDL_DISABLE_IMMINTRIN_H -dGL_GLEXT_PROTOTYPES; without the two defines you get a bogus avx512fintrin.h/MAX_PROC_PARAMS error that is not the real wall. DO NOT QUOTE AN app.py LINE NUMBER -- the owner edits it live; it moved 1195 -> 1217 -> 1233 -> 1240 in one evening. PREVIOUS LEAD, 2026-09-11 evening, SUPERSEDED: `pascal26 --threadsafe -dSDL_DISABLE_IMMINTRIN_H -dGL_GLEXT_PROTOTYPES lekkerzeilen/__main__.py` now walls inside app.py's App.open_region: `annotate the type / too dynamic [a=11 b=22]` (tyInt32 vs tyVariant) on `self.world = scene if isinstance(scene, world.World) else None`, filed as bug-n-a-field-assigned-a-class-or-none-in-two-methods-wont-widen and NOT YET REDUCED -- the identical construct on the same field compiles ~600 lines earlier in the ctor, and four reductions all pass. THREE WALLS CLEARED THAT EVENING: the ctypes wall in capture.py (corpus seam rewrite, deliberately uncommitted in /home/neo/lekkerzeilen); `world.label(...)`, which was a COMPILER bug and not a corpus one (55981bc63 -- a unit-qualified member named like a Pascal reserved word was rewritten to a trailing-underscore spelling no Python module declares, so it missed by construction; f004e408e did the class sibling); and `import threading`, which needed no code at all, only the --threadsafe flag its own diagnostic prescribes. The two -d defines are required for the native backend's C headers and are not optional. DO NOT QUOTE AN app.py LINE NUMBER -- the owner edits that file live and the wall moved 1195 -> 1217 within an hour. SUPERSEDED LEAD, 2026-09-11 morning: THE ENTRY-POINT CLOSURE IS ONE WALL FROM COMPILING, measured 2026-09-11 at 7958322f8 / binary 465845b20d1e: `pascal26 --threadsafe lekkerzeilen/__main__.py` emits 27 lines and EXACTLY ONE error, `capture.py:44 no member create_string_buffer came of the qualifier ctypes`. The 28-of-35 ratio below counts modules AS SUBJECTS and includes dead code (nothing imports gfx.py; four of the five ctypes files sit in a backend arm pxx never takes) -- the closure is goal 4's question and the ratio is not. Behind that line, measured in a scratch copy, the next wall is one line further (`gl.ReadPixels`, 7 args against the 4-arg run-time-dispatch cap, filed as feature-n-a-runtime-dispatched-method-call-is-capped-at-four-arguments) and BOTH WALLS ARE THE SAME CAUSE: `_pxx.py`'s `class gl:` declares five names and neither of these, so writing the backend clears both at once. AND THE FORK IS NARROWER THAN STATED BELOW -- THE DEMO NEEDS NO FFI. capture.py names ctypes for one thing, a writable byte buffer for glReadPixels read back via .raw, and the owner's own `_pxx.py` docstring specifies the native backend as having 'no CDLL loading, no restype/argtypes declarations, no create_string_buffer'. So goal 4 waits on task-b-write-the-lekkerzeilen-pxx-platform-backend, not on a ctypes decision; the FFI question is real for other programs and is not this umbrella's. The seam's probe is CORRECT (verified: `try: import ctypes / except ImportError` takes the except arm under pxx), so do NOT shim one member to clear the line -- that flips the probe, which is the 27 -> 25 measurement below. PREVIOUS LEAD, still accurate as a module ratio: RE-MEASURED 2026-09-11 at compiler f9fb672ee109 / corpus 2a3d60e: **28 of 35 compile and CTYPES IS THE ONLY WALL LEFT** -- all seven remaining failures are it. Every count and cause-list later in this summary is from 2026-09-10 and is SUPERSEDED; they are kept because the per-module reasoning is still the best record of how each wall was characterised. What cleared since: the `_pxx` module-as-a-value wall (frankZ's getattr-over-a-unit-alias fold plus frankuser's corpus seam rewrite), heapq (frankuser), and the import-order arity refusal on `nearest` (frankB, 8de1fff93, which took hud.py and traffic.py). threading and sqlite3 no longer appear as walls under `--threadsafe`. SO GOAL 4 NOW REDUCES TO ONE QUESTION AND IT IS NOT A COMPILER QUESTION: do we want NilPy programs to be able to call a native library the way CPython programs do, or do we want this app to reach the native layer through pxx's own binding mechanism and change its source to suit? The umbrella already answers it one way at 'do not build mimic_ctypes for this target' below, and frankuser MEASURED the cost of the other way on 2026-09-11: a correct partial mimic_ctypes moved the census 27 -> 25, because `import ctypes` succeeding IS A CAPABILITY PROBE -- the app's seam commits to the arm that needs the full FFI the moment the import resolves, so a partial shim is not partial progress. Owner-set target (2026-09-08): the lekkerzeilen sailing simulator -- /home/neo/lekkerzeilen -- as a REAL-WORLD nilpy target. It was written knowing about pxx and it shows: the runtime package imports ZERO third-party libraries (numpy and PIL appear only under tests/ and tools/), there is not one f-string in it, and no async, yield, match, walrus or annotation. RE-MEASURED 2026-09-10 at compiler a812b9549413, tree 813c99cc7: 22 of 35 modules compile clean. THE CORPUS MOVES UNDER YOU -- the owner renamed flight.py to drone.py at 15:47 and added atlas.py at 16:30 the same day, so it was 34 modules that morning and a delta between two censuses is not attributable to compiler work unless you check. The 13 remaining walls are FIVE causes, and THREE OF THE THIRTEEN ARE BY DESIGN AND OWE THE COMPILER NOTHING: platform/{_ctypes_backend,_gl,_sdl2} are the CPython arm of the app's own two-backend seam, which under NilPy is never imported at all -- `try: import ctypes / except ImportError: from . import _pxx` takes the _pxx branch, measurably so since 708555fdb, and they only appear as walls because the census compiles every file DIRECTLY. Of the ten that remain: capture.py and gfx.py need ctypes, and CALLING THAT A SEAM EDIT WAS WRONG -- corrected 2026-09-11 (frankB measured, confirmed here): gfx.py uses `ctypes.` SIXTY times across NINE names (byref 21, c_uint 19, c_void_p 5, sizeof 4, c_float 4, c_int 3, create_string_buffer 2, c_char 2, c_char_p 1) plus the `(TYPE * N)(...)` array-type constructor at three sites, and 22 of its 148 `gl.*` call sites marshal through it -- it IS the OpenGL marshalling layer. Routing the import through a try/except makes the module COMPILE and leaves it unable to do the one thing it exists for: the census moves by two and the demo moves by zero. So these two rows are ONE MISSING CAPABILITY (a bounded mimic_ctypes: nine names and an array-type constructor, not all of CPython's ctypes), not two module fixes, and anyone who does the seam edit anyway MUST say in the resolution that ctypes is not solved or two cleared rows will read as the capability landing; threading blocks 3 modules on 2 real sites (__main__ reports gauges.py line 36 through the import chain); sqlite3 blocks 2 subjects on ONE site at world.py:188 (atlas.py:188 is `if box is None:` and has no sqlite3 near it); a module as a VALUE blocks platform/__init__.py with bindings.py cascading behind it; and a field from a qualified module constant blocks traffic.py:402. So the COMPILER owes 8 modules on 4 causes, not 13 on 5. READ THE PER-MODULE LIST, NOT THE COUNT: clearing the *unpack-with-defaults wall moved five modules and only three of them went green -- the other two advanced into a SIGSEGV that was invisible behind it. TWO STANDING RULES FROM THE OWNER, both unusual and both deliberate: (1) WE MAY CHEAT ON THE SOURCE -- where something is principally incompatible with nilpy, changing lekkerzeilen is allowed, which is the opposite of the usual corpus rule; (2) it is NOT to be wired into the test suite, like uforth. It is a target to attempt, not a gate.

## 2026-09-12 — THE CLOSURE COMPILES AND RUNS, and the last wall was a RUNTIME one

The headline is in the summary. What belongs here is the part a changelog row
loses.

**The three walls were found by RUNNING the target, and only one of them was on
this umbrella's list.** `io.open` was new code in a file nobody had looked at;
`__doc__` was the banked ticket and the ticket named the wrong file; the `**`
fault was not on any board anywhere, because nothing that compiles a census can
see it.

**This is the aperture change worth recording.** Every census in this ticket
above — six of them — asks *does it compile*. The moment the answer was yes, that
instrument had nothing left to say, and the next defect was a `ValueError` raised
at import time by a module-level table. A first-failure compile census cannot
observe a runtime fault at all; it is not a weak instrument for the question, it
is the wrong instrument. The replacement is free: run the binary.

**Two predictions of mine were falsified, and stating them first is what made
them useful:**

1. I predicted `open(p, encoding="utf-8")` would silently bind `encoding` as the
   MODE. It is LOUD — `undefined variable (encoding)`. So the wall was a missing
   function, not a silent wrong value.
2. I predicted that a `**` in the MAIN module would make the imported module's
   `**` correct, since the hook is one runtime global. It did not: a unit's
   initialisation runs before the main body that assigns it. That falsification
   is the second half of
   `bug-n-an-imported-module-s-star-star-never-installs-pypowhook`, and without
   the prediction it would have read as one defect.

**Eliminated suspects, each measured rather than reasoned about,** because the
`**` fault presented as "lekkerzeilen segfaults somewhere": `audio.py:40`'s GAIN
expression (identical to CPython); `0.0 ** 4.56` in a MAIN module (correct —
which is what hid it); every variant/static operand combination for `**`
(correct); the whole nested-closure structure of `lines.py`'s `station(t)`
reproduced in a main module (correct); and `PyExprMode` being false inside an
imported module (disproved — Python `and`/`==` precedence is in effect there).

**The instrument note, and it is the owner's:** I was about to temporarily tag
`pylib.pas`'s six `math domain error` messages to find which function raised.
*"you realize you can build with debug params?"* — `-g` plus gdb gave the
location in one shot. Worth knowing what `-g` does NOT give here: no symtab, and
no `.pas` files in the line table, only the app's `.py`, so a backtrace past the
exception-handler frame is garbage (the handler has no CFI and gdb scans to a
stale return address). It reported a plausible wrong function with a plausible
wrong line; stepping was the reliable evidence.

**What is NOT established.** Only `--help` has been run. The simulator itself
still waits on `task-b-write-the-lekkerzeilen-pxx-platform-backend`, and
...
## 2026-09-12 — `--probe` passes, `--starts` is a SILENT wall

Two more non-graphics modes run. Taken together with `--help` and `--conform`,
four of the five no-window paths have now been exercised.

**`--probe` passes.** It loads the SDL2 and libGL bindings without opening a
window and prints `bindings load cleanly.` Its only difference from CPython is
the one line that is supposed to differ:

```
-backend : ctypes        (CPython)
+backend : pxx           (pxx)
```

That is the app's own two-backend seam choosing correctly, and it is the first
direct evidence in this ticket that the `_pxx` arm *loads real libraries*, not
merely that it compiles.

**`--starts` does not, and nothing says so.** Under CPython it lists the `rijn`
region and two places to start; under pxx it prints `open water: nowhere in
particular, which is the point` and exits 0. The world directory is not found.

Found by stepping the resolution chain with prints, after a standalone probe was
misleading: `from lekkerzeilen import world` (an absolute package import, which
the app does not use) binds NO members at all under pxx — `world._home` is "no
member ... came of the qualifier". That is possibly its own gap and is **not** the
cause here; the app uses `from . import world`, which binds fine. Recording it so
the next reader does not mistake the probe artefact for the finding. The real
chain was found by instrumenting a scratch copy of the package.
