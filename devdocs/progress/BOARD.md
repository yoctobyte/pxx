...
| --- | --- | --- | --- | --- | --- |
| perf-a-every-npy-compile-still-rebuilds-the-whole-nilpy-runtime | A | 85 | perf | The structural half of bug-a-every-nilpy-compile-pays-a-fixed-nine-second-cost, which halved the constant (8.62s -> 4.06s) by removing the hotspots but did NOT remove the WORK: every .npy compile still parses and code-generates all 24,460 lines of pylib.pas + pyeval.pas before it looks at the user's program. A zero-byte .npy costs 4.06s where `begin end.` costs 0.24s. | — |
...
| bug-n-a-comparison-dunder-against-a-non-class-operand-answers-wrongly | N | 70 | bug | `obj == 1` and `obj < 1` do not call the class's __eq__/__lt__ when the right operand is an INT — they answer False regardless of what the method returns. A __eq__ that returns True is simply not consulted, so an equality test silently answers the opposite of CPython, with no inheritance and no error. | — |
...
| bug-nilpy-a-keyword-call-through-a-statically-unknown-callee-does-not-compile | N | 60 | bug | `a = mk(1); a(x=5)` fails to COMPILE — `error: undefined variable (x)` — because the keyword-argument lowering only fires when the frontend can resolve the callee to a def/lambda by name. The runtime dispatcher handles this fine (a callee reached as a PARAMETER works), so it is the parse-time gate, not the call path. | — |
| bug-nilpy-a-lambda-returned-directly-is-not-callable | N | 70 | bug | `return lambda x: ...` yields a value that is NOT callable — `TypeError: object is not callable (no __call__)` where CPython calls it. Binding the same lambda to a local first (`g = lambda ...; return g`) works, so the lift is fine and it is the RETURN of the lambda expression that loses the callable tag. | — |
...
| bug-nilpy-empty-str-and-none-are-the-same-value | N | 75 | bug | `\"\" is None` answers TRUE for a NilPy str: Pascal's empty AnsiString IS a nil handle, so the None sentinel and the empty string are indistinguishable — contradicting pylib's own comment that they are not. | decide-nilpy-none-str-sentinel-vs-textstr-kind |
...
| bug-p-array-of-const-integer-arm-picks-the-int64-overload | P | 55 | bug | fcl-json's `CreateJSONArray([1])` yields a `TJSONInt64Number` where FPC yields a `TJSONIntegerNumber`. The direct `CreateJSON(1)` agrees with FPC; only the `array of const` path diverges, in `With Element do case VType of vtInteger: CreateJSON(VInteger)`. Two of the fpjson suite's 203 cases fail on it. | — |
...
| compat-pascal-binop-operand-eval-order | A | 3 | compat | pxx evaluates binary-operator operands (and CALL ARGUMENTS) left-to-right; FPC evaluates right-to-left | — |
...
| compat-pascal-method-impl-without-declaration | P | 8 | compat | `TC.Foo` implementation for a method the class never DECLARED compiles (FPC rejects) | — |
| compat-pascal-not-of-a-cast-constant-keeps-its-width | P | 3 | compat | `not Byte(0)` folds to 255 in pxx and to -1 in FPC — FPC evaluates a constant `not` in the Int64 domain and drops the cast's width. pxx matches Delphi. Variables agree; only the constant-folded form differs. | — |
...
| feature-nilpy-staticmethod-and-classmethod | N | 70 | feature | `@staticmethod` and `@classmethod` are rejected | — |
...
| feature-opt-emitasmx64-reparses-fixed-strings | O | 60 | feature | `EmitAsmX64` re-parses the same hardcoded assembly strings on every compile — ~12% of a NilPy compile | — |
...
| feature-p-const-evaluator-carries-unsigned-64-bit | P | 48 | feature | `High(QWord)`, `Low(UInt64)`, `High(NativeUInt)` and `High(PtrUInt)` are rejected at compile time — the const evaluator carries Int64, which cannot hold 2^64-1. Every other integer type name folds. Idiomatic FPC code that spells a machine-word bound this way does not compile. | — |
...
| decide-nilpy-none-str-sentinel-vs-textstr-kind | U | 40→75 | decide | Re-ask of decide-nilpy-none-str-representation: the chosen fix (a NilPy string kind whose blocks may be zero length) rests on a block kind that nothing in the tree ever stamps, so it is a Track A representation project rather than a bugfix. A None SENTINEL closes the reported bug at a fraction of the cost — but closes less. | — |
...
2395 ticket(s) — full table in [`BOARD-done.md`](./BOARD-done.md), generated alongside this file.
...
- [p 58] [P] feature-p-class-helper-for-a-class-type
...
- [p 45] [P] compat-pascal-supports-three-arg-out-form
- [p 45] [P] compat-pascal-uses-sysutils-withdraws-the-variadic-concat
...
- [p 45] [N] regression-lib-test-lib-mimic-xml-etree-elementtree-2
...
- [p 30] [S] feature-esp-peripheral-callback-api
...
- [p 25] [A] bug-a-a-riscv32-diagnostic-names-the-wrong-target
- [p 25] [A] bug-a-numeric-goto-labels-are-not-supported
- [p 25] [A] bug-a-riscv32-codegen-has-no-variant-support
...
- [p 25] [O] feature-opt-arch-level-and-dispatch
...
- [p 20] [D] task-d-document-the-strict-overload-width-flag
- [p 20] [D] task-d-document-warn-ignored-directives
...
- [p  8] [P] compat-pascal-method-impl-without-declaration
- [p  8] [A] compat-pascal-strict-fpc-abs-and-sqr-widths
- [p  5] [P] compat-pascal-directive-in-comment-ignores-nested-comments-off
- [p  5] [A] compat-pascal-strict-fpc-should-reject-a-duplicate-identifier-in-one-scope
...
- [p  3] [A] compat-pascal-binop-operand-eval-order
- [p  3] [P] compat-pascal-not-of-a-cast-constant-keeps-its-width
