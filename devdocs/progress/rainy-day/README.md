...
## Float went to its own scope, not here (2026-08-19)

Float accuracy was briefly parked here and then moved to **`devdocs/progress/float/`** at the
owner's instruction — it deserves its own scope rather than sharing one with OS ports, big
infra and design parks. Do not park float tickets here; see `float/README.md` for what counts
and, more importantly, for what does not.
