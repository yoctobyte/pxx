...
title: "BOTH ARMS DISCHARGED 2026-09-20; what remains is two UNSPRUNG traps, one now guarded — a mimic shim under a colliding name would be silently unreachable"
track: N
prio: 30
type: bug
blocked-by: []
...
## RE-MEASURED AT HEAD 2026-09-20 (frankb-8e) — EVERY LIVE ROW WORKS, THE OWNER'S SPECIFIED FIX IS BUILT, AND WHAT IS LEFT IS TWO UNSPRUNG TRAPS — ONE OF THEM NOW GUARDED

Compiler `f94c2a7e2396d2be`, pxx HEAD `a852664bb`. Oracle: CPython 3.14.4. Re-measured rather than read, because this ticket has carried a hazard block since 09-10 and **a warning decays like a lock: silently, in the direction of doing nothing, for as long as it is trusted.**

**ARM 2's LIVE ROWS, ALL PASSING:**

| row | pxx | CPython |
| --- | --- | --- |
| `zlib.compress` / `zlib.decompress` round trip | `True` | `True` |
| `zlib.InflateZlib` as a member | binds | — (Pascal-only) |
| `from zlib import InflateZlib` | binds | — |
| `zlib.decompress(b"\x78\x9c\x00")` (short stream) | `zlib.error` | `zlib.error` |

**THE CONCRETE MISBEHAVIOUR ROWS THIS TICKET LISTED ARE DISCHARGED.** `import random` compiles (the body records `undefined variable (random)`). `platform`, `textfile` and `typinfo` now give a **loud, explanatory refusal that names the remedy** — *"platform is the Pascal unit .../lib/rtl/platform.pas, not a Python module — a bare NilPy import resolves to Python (.py/.npy) only. To reach the Pascal unit, name it with its extension: import 'platform.pas' as platform"*. That is the correct answer for a unit that is not a Python module, and it is the opposite of the silent wrong binding this ticket was filed about. `math`, `re`, `json`, `base64`, `html` all compile.

**THE PER-MODULE PREFERENCE TABLE THE OWNER SPECIFIED IS BUILT, AND THIS TICKET DOES NOT SAY SO.** His 2026-09-10 ruling was *"and importing preference whitelist we can just hardcode. 'zlib? -> rtl zlib unless...'"*. That is `PyRtlUnitServesPython` (`compiler/pasparser_proc.inc`), **18 entries**: ast atexit base64 collections configparser html io json markdown math pathlib pil random re subprocess tempfile tkinter zlib. **`zlib` entered it in `80d71d782`** (*"zlib round-trips, and `import zlib` stops binding the host header"*), which is what moved the rows above.

**AND THE BODY OF THIS TICKET IS STALE ABOUT IT.** The section above says *"`pathlib` is in it and `zlib` is not, and both are fine"* — true when written and false since `80d71d782`. It is left in place as the 09-10 report it was, flagged here rather than edited, because it is the reasoning that correctly sent its author to `ParseUsesUnit` and the record of that is worth more than a tidy line.

**THE MIMIC-SHIM PRECEDENCE TRAP IS STILL UNSPRUNG — re-censused with the populations stated**, since a bare count is not re-derivable. Measured from the filesystem, not a list: **163 `.pas` under `lib/`, of which 23 are `mimic_*` shims and 137 are unit names**, plus the **18** curated names above. **Collisions: 0.** (The 09-19 row recorded 34 and 135 against my 23 and 137 — **both are carried, neither replaces the other**; that census counted something else, most likely module names a shim serves rather than shim FILES, and the disjointness verdict agrees either way.)

**THE HALF THAT NOW HAS AN ALARM.** A trap with a named spring condition and no detector is a hazard block waiting to go stale, so it got a detector: **`tools/mimic_shadow_check.py`, wired into `tools/gate.sh quick`.** It asserts that no `mimic_X.pas` is shadowed by either ours-first substitution — a same-named unit, or a `PyRtlUnitServesPython` entry — because `--no-shims` lifts both and checking one would pass a tree where the other hides it. **Its criterion is set DISJOINTNESS and it records no baseline count**, deliberately: a guard that pins "23 and 137" reds on the next unit anyone adds, for no defect, and a guard that cries wolf on its first outside run teaches that it can be ignored. `--selftest` proves it can fail in **both** directions **against the real code path** — its first version asserted against a local copy of the comparison, which would have kept printing PASS had the checker itself broken.

**WHAT IS LEFT, AND WHO OWNS IT:**

1. **The routine-level collision hazard — UNSPRUNG AND UNGUARDED, and it stays here.** A Pascal routine whose name collides with a Python one, whose signature ACCEPTS CPython's argument shape, and whose SEMANTICS differ, returns a wrong value with no diagnostic. Measured 09-19 over 74 outcome comparisons: zero silent divergences, every failure a loud compile-time refusal. `Ldexp` is the near miss — two of the three properties, saved by the third. **My guard does not reach this**: it is module-level, and this is routine-level.
2. **The host-C-header fallthrough** is NOT this ticket's: [[bug-n-a-bare-nilpy-import-falls-through-to-a-host-c-header-of-the-same-name-and-says-nothing]], p35, open. The compiler's own comment at `PyRtlUnitServesPython` routes it there too.

**RE-RANKED 85 -> 30, AND THE REASON IS THE SAME MECHANISM I SPENT THE DAY ON.** The 85 was set when arm 1 was a wall on the top-ranked target. **Arm 1 was fixed 2026-09-11 and arm 2's deliverable landed in `80d71d782`; nobody re-ranked, so the number outlived both arms** and sat at the top of Track N pointing at two traps with measured-zero populations. This is a third field in one day where partial closure left a mechanical record true-looking and stale — after a `blocked-by` edge on the lekkerzeilen umbrella and an inherited `effective_prio` (`d4ce89f5a`). **The summary here was exemplary throughout and did not prevent it**, exactly as in those two: the summary is what READERS read, and `prio:` is what the RANKER reads.
