...
type: feature
blocked-by: []
summary: "RE-MEASURED AT HEAD 2026-09-19 AND THE COUNT IS NO LONGER TWELVE: three of the twelve now COMPILE AND AGREE with CPython -- atan2, expm1 and ldexp -- and `ldexp` was one of the four this ticket said could land today, so it landed and nobody recorded it. EIGHT are still absent (isqrt, log1p, asinh, acosh, atanh, isfinite, dist, remainder). The twelfth, `frexp`, is NOT absent and never was a missing name: it resolves to the PASCAL `procedure Frexp(const x; var mantissa; var exponent)` in lib/rtl/math.pas and fails with `no overload of frexp matches these arguments`, because CPython returns a PAIR. That is a wrong-carrier defect, not a gap, and it is the same mechanism as the zlib arm of bug-n-a-same-named-rtl-unit-shadows-both-a-relative-import-and-a-mimic-shim: a NilPy `import math` binds the Pascal unit and resolves names against it case-insensitively, so a Python spelling is served by a Pascal routine whenever one happens to bear the name. Where the two signatures agree you get a silent right answer (ldexp); where they do not you get a confusing refusal (frexp). CORRECTION 2026-09-19, and it reverses a claim this summary carried for an hour: I reported lib/rtl/math.pas:131's 'FOUR NAMES ARE DELIBERATELY ABSENT -- pow, log, copysign and atan2' as STALE because all four compile and agree in NilPy. THE COMMENT IS CORRECT AND I MEASURED A DIFFERENT CLAIM. It is a statement about math.pas, where `grep` answers ZERO declarations for all four exact spellings; what exists there is `Power` and `ArcTan2`, which are different NAMES. All four reach NilPy through the compiler's rename/intercept table in pyparser.inc instead -- math.pow -> Power (:13910), math.log -> Ln (:13882), math.atan2 -> ArcTan2 (:13840), math.copysign -> pymath_copysign and math.trunc -> pymath_trunc (:13826-7) -- which the comment ITSELF says in its last line ('NilPy gets intercepts for those three instead'). ITS TRAP IS LIVE AND CALLING IT STALE IS THE EXPENSIVE DIRECTION: pxxcio is auto-pulled into every C program and does `uses math`, so adding pow/log/copysign to math.pas hijacks libc's in every C program -- measured once already, a C program answered pow(2,10)=1 and copysign(3,-1)=0.785398. Do not retire that line. Caught by frankuser, who measured it rather than taking my word. CONSEQUENCE FOR THE GROUP CLAIM BELOW: atan2 is NOT an instance of the case-insensitive Pascal-unit binding; it is a compiler rename. The three that genuinely are that mechanism are expm1 (ExpM1), ldexp (Ldexp) and frexp (Frexp). The remaining eight inherit the standing 'do not map a 1-ulp-off RTL routine' policy. NOTE expm1(1.0) differs from CPython in the last ulp (1.7182818284590455 vs 1.718281828459045); per the F-lane rule that is float noise and not a defect to chase."
---
