...
**THE REMAINING WALL IS AN RTL GAP, NOT A FRONTEND ONE, AND ITS COORDINATE
LIES.** `undefined variable (ENotSupportedException)` — the compiler's `in:`
line says `pscanner.pp` and the construct is at **`pparser.pp:778`**,
`raise ENotSupportedException.Create(SErrMultipleSourceFiles)`. `lib/rtl` does
not declare that class. It is in the BASELINE's five errors too, so it is
pre-existing and was hidden behind the CurSourcePos cascade in the reading, not
in the output. Track B/RTL, one declaration; do not spend a P seat on it.
