...
owner: frankB
summary: "PHASE 2 IS ONE BACKEND FROM DONE: six of seven converted (x86-64, aarch64, arm32, riscv32, xtensa `fe8662e24`, wasm32 `0973746b0`); **i386 alone remains**. Read `TargetHasByteStrPrefixCodegen` in `util.inc` for that count and never a commit message -- wasm32's own says `STILL OPEN: i386 and xtensa` and xtensa was already in. THE DEFECT THIS PHASE WAS WRITTEN AROUND IS A CLASS, NOT ONE BUG, AND IT IS NOT CLOSED. Asked whether the walker was one fix or the first of a class, the answer measured out as a class: (1) the WALKER, shared, `ir.inc` -- `p^` for `p: ^string[10]` lowers to a bare pointer LOAD tagged tyPointer, matched no arm, fell to the tyString default; the pointee's kind was never missing, it is `PtrElemTk` on the pointer's own symbol and nothing asked. (2) x86-64 COMPARISON -- `EmitStrCmpReg` (`symtab.inc`) had no kind in its signature at all and read both lengths at 8. (3) arm32 COMPARISON -- its extractor was already width-aware and its four callers passed `IntToTypeKind` straight past it. (4) x86-64's `IR_STORE_MEM` arm was never converted and read its destination kind from a STALE `symIdx` belonging to another arm. Fixed in `764dc3a30`; a walker-only fix would have closed ONE of the four and looked complete. TWO READERS SURVIVE IT AND A FIFTH CAUSE EXISTS (`40646620c`, all four converted backends): `p^[1]` reads BLANK while `s[1]` and `r.f[1]` read `h` in the same run -- the index origin follows the prefix wherever the symbol is reachable and not through a bare pointer, the walker's shape again at a second site; and `r.f = 'hello'` SEGFAULTS on x86-64/riscv32 and returns FALSE on aarch64/arm32 while `s =` and `p^ =` are green beside it, so the comparison arm now resolves a variable and a deref and still not a FIELD. Both are ticketed. THE FLAG ROWS ARE DELIBERATELY UNWIRED because of them -- unwired is the honest state, not an omission. THE DECISION WAITING FOR THE OWNER, and it is a substrate question rather than a bug: THREE sites now carry a comment telling the next author not to read a width from `IntToTypeKind` (aarch64 `IREmitNodeAarch64`, riscv32 `EmitFrozenStoreRISCV32`, arm32 `EmitArm32StringParts` -- named by CONSTRUCT, not line, because this ticket has already watched three line citations go stale), arm32 VIOLATED ITS OWN COMMENT at four call sites, and `IRStrTkOf`'s docstring already prescribes the remedy verbatim. Two is a smell, three is a design flaw, and a comment is not a mechanism. So: should asking a prefix its width stay a per-site decision at all, or does the IR owe callers a resolved kind so the question cannot be got wrong? `43c31eff0` fences the pattern from Track T, which buys time and does not answer it. P4 (THE FLIP) IS UNRELEASED and serialises the fleet -- it re-types every string in the compiler and is judged against the tree it lands on, so it goes last with nothing else in flight; its definition of done includes DELETING `-dPXX_SHORTSTRING`. NEW P4 PRECONDITION found in review: wasm32 is converted with ZERO Makefile rows passing a mode flag (arm32 36, riscv32 31, aarch64 29, xtensa 6, wasm32 0 of 22), and its one standing shortstring row runs at DEFAULT -- by that commit's own byte-identity proof, precisely the path it did not change. Today that is a gap in a path few programs take; after the flip that backend becomes an unverified DEFAULT, so the window closes at P4. STILL TRUE FROM P1: scope by the 128 `TypeIsFrozenString` sites (28 touch a layout number), never by `tyShortString`'s 63 name-mentions, which point the wrong way; `tyFixedString` stays as-is for N>255; this is additive; and the self-host exposure was ONE declaration (`lib/rtl/typinfo.pas`'s `TRttiStr`, resolved), which takes `lib/rtl` out of the flip commit entirely. Narrative in `devdocs/progress/HANDOVER-shortstring-night.md`; this ticket's body is append-only history."
---
