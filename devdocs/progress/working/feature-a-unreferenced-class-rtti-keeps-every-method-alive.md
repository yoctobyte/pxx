...
## 2026-09-22 (frankb-8e) — the headline bucket is 94.7% NAME-REFLECTION, so RTA is the wrong mechanism

**This answers the ticket's own standing question** — *"when can a VMT slot be
proven UNDISPATCHABLE?"* — by showing that almost none of the residue is in a
VMT slot at all. The bucket was five unrelated mechanisms added together.

### The measurement

Same subject, same ISA, same flags as the rows above, at `c52db169f` + the
held stage-2 patch (local binary `fc266762b149`):

```
./compiler/pascal26 --target=xtensa --xtensa-abi=windowed --xtensa-long-calls \
  --platform=esp --no-signals --dce --dce-why --emit-obj \
  examples/esp32/nilpy-c3/main/main.npy <out>
```

| split of the old `vmt/rtti slot` bucket | bytes | bodies | share |
| --- | ---: | ---: | ---: |
| **rtti method table (reflected by name)** | **165,059** | **143** | **94.7%** |
| nilpy signature word (per-proc) | 8,289 | 1 | 4.8% |
| interface method table slot | 780 | 3 | 0.4% |
| **vmt dispatch slot** | **222** | **2** | **0.1%** |
| sum | **174,350** | **149** | |

**The sum is byte-identical to the one-bucket figure** measured minutes earlier
on the same tree with the stage-1 binary (174,350 B / 149 bodies, total live
833,482 B). That is the control: the split is reporting-only and loses nothing.

### What it means for the pass

**Rapid Type Analysis does not reach this.** RTA proves a class is never
instantiated — all its constructors dead — and then drops its dispatch slots.
Dispatch slots are **222 bytes, 0.1%**. The mass is `TMethInfo.Code` words,
which `PyClassRefNew` (`compiler/builtin/pyeval.pas:3606`) walks **by name** to
find `create`, and a name lookup is not a dispatch edge: proving the class is
never constructed *statically* says nothing about whether a string reaches it.

This CONFIRMS and explains the summary's existing line — *"the classes holding
those slots are all LEGITIMATELY LIVE (TPyList, TPyDict, TPyFile), so an
unreferenced-CLASS criterion reaches none of it."* That was right, and the
reason is now measured rather than inferred.

**So the next instrument is about reflection reachability, not instantiation.**
The question to answer is which RTTI method entries can be reached by a name
that some live code can actually produce. `PyClassUsedAsValueEx`
(`pyparser.inc:18568`) already does the NilPy half of exactly this shape — a
conservative token scan for a class name in value position, closing over
ancestors and descendants — and is the precedent to read first.

Cross-measured on x86-64/posix, same program, for anyone who wants a second
ISA: 85,106 B / 143 bodies reflected-by-name against 190 B / 2 in dispatch
slots. Same conclusion, same body counts, different bytes.

### Two corrections this section makes to the ones above it

1. **The recorded command DOES build on xtensa**, and the section above saying
   it "does not build at HEAD" is about FLAGS, not about the target. Default
   Call0 overflows (`addi immediate displacement`); `--xtensa-abi=windowed
   --xtensa-long-calls` clears it; the IDF profile then needs `--emit-obj`
   because it emits an object for the IDF link rather than a complete
   executable, and its diagnostic says so. An empty `.npy` and `print(1)` both
   build that way (975,588 B / 975,740 B). Briefly believed by two seats to
   mean NilPy does not build for xtensa at all.
2. **`--xtensa-long-calls` was not the whole remedy** — `--emit-obj` is the
   other half, and the section above stops at the long-calls error.

### Stage 2 is held, and how to restore it

The split needs a `Kind` field on `TMethodFixup`, which is a BUILTIN record:
field access resolves through the tables in `symtab.inc`, not through the
declaration in `defs.inc`, so **the pinned compiler cannot compile a tree that
writes it** and `gate.sh` seeds the fixedpoint from the pin. Stage 1
(`c52db169f`) declares and registers the field and uses it nowhere — that is
pin-compilable and landed. **Stage 2 is three hunks and is inert until a pin
carries stage 1:**

1. `emit.inc`, in `AddMethodFix`: `MethodFixups[MethodFixCount].Kind := kind;`
2. `dce.inc`, the `MethodFixCount` root loop: replace the single
   `DceWhyNow := DCE_WHY_VMT;` with a `case MethodFixups[i].Kind of` mapping
   `MFK_IMT/MFK_RTTIPROP/MFK_RTTIMETH/MFK_PYSIG` to their `DCE_WHY_*` codes,
   `else DCE_WHY_VMT`.
3. `dce.inc`, `DceWhyName`: `DCE_WHY_VMT` becomes `'vmt dispatch slot'`, and
   `test/test_dce_why_root_report.pas`'s expected string in the Makefile moves
   from `[vmt/rtti slot]` to `[vmt dispatch slot]` **in the same commit** —
   `TDerived.Tag` is a genuine dispatch slot, so that fixture gets *stronger*,
   not merely updated.

Do not rename in stage 1: until the field is readable every mechanism still
lands in that bucket, so the narrower label would count more than it names.
