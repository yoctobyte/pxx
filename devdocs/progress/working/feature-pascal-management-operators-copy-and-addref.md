...
- ~~Whether the two hooked assignment arms are the whole population~~ —
  **answered 2026-09-06: no.** 23 `IRAppend(IR_COPY_REC*)` emit sites against 2
  hook sites; censusing 14 copy SHAPES against fpc found 13 agreeing and one
  diverging — a whole-record assignment of a record that CONTAINS a
  Copy-operator field runs nothing, while `Initialize`/`Finalize` reach the same
  field through the scope desugar. Filed as
  [[bug-a-a-whole-record-assignment-does-not-run-a-contained-fields-copy-operator]].
  The copied VALUE is correct, so it is invisible to an output comparison — the
  census had to make the operator print.
