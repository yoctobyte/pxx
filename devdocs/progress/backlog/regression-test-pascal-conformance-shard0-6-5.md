...
- 2026-09-09 (frankS) — a SECOND instrument agrees the row is dispositioned at HEAD, and it is not the sharded tier: a full unsharded `tools/run_pascal_conformance.sh` over all 550 at compiler `7d1893a9a54e` reports **428 pass, 0 fail, 72 skip, 50 auto-gated**, with `tgenfunc13` among the skips. Zero failures corpus-wide, so nothing is waiting on this row. Recorded rather than acted on — the ticket says do not claim and that closing it is a human's call, and a whole-corpus green is evidence for that call, not a substitute for it.
