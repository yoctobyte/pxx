...
- 2026-09-10 — the seven watcher saw `size-canary#src:tools/size_canary.py` GREEN at dafc9b2f4d88 (tier native) and did NOT close this: this is a repeat stub (`regression-size-canary-size-canary-2`, not `regression-size-canary-size-canary`) — the job already went red, was closed, and came back, so one green is the outcome a live intermittent bug produces most of the time. The green is recorded because it is evidence and because a ticket that stops moving with no reason reads as forgotten; closing this one is a human's call.
- 2026-09-21 — the borg watcher saw `size-canary#src:tools/size_canary.py` GREEN at edf1ecf4bfe8 (tier native) and did NOT close this: this is a repeat stub (`regression-size-canary-size-canary-2`, not `regression-size-canary-size-canary`) — the job already went red, was closed, and came back, so one green is the outcome a live intermittent bug produces most of the time. The green is recorded because it is evidence and because a ticket that stops moving with no reason reads as forgotten; closing this one is a human's call.
- 2026-09-24 — the borg watcher saw `size-canary#src:tools/size_canary.py` GREEN at 8e2464989f2a (tier native) and did NOT close this: this is a repeat stub (`regression-size-canary-size-canary-2`, not `regression-size-canary-size-canary`) — the job already went red, was closed, and came back, so one green is the outcome a live intermittent bug produces most of the time. The green is recorded because it is evidence and because a ticket that stops moving with no reason reads as forgotten; closing this one is a human's call.
