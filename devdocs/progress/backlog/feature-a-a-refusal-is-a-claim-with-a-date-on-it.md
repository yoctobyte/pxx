...
## 210 — THE WRONG VALUE'S ARITHMETIC NAMES *WHICH* WRONG VALUE IT IS

*(pxx-a5, 2026-08-30, `pxx-self_unitrec` — second `-O3` wrong-code bug in two directed slices,
reduced to eight lines.)*
...
`pxx -O0/-O1/-O2 = 635218`, `fpc -O0/-O2 = 635218`, **`pxx -O3 = 222`.**

**222 is arithmetic, not noise, and that is what turned a divergence into a diagnosis.**
`X * 31 xor 7 = 222` solves to `X = 7` — which is `c`. So the value entering the second
statement is **the third variable**, not the first statement's result.

Then the move that separates two bugs which look identical from one sample: **set `b := 5`.**
`-O2` and FPC move to 635317; **`-O3` still prints 222.** The `-O3` answer does not depend on
the dropped statement's operands at all — which distinguishes *"statement one is dropped"* from
*"statement one is miscomputed"*, and no amount of staring at 222 alone would have.

> **A wrong number is data. Solve it.** A wrong value that can be inverted tells you which of
> the program's values it *is*, and a second sample chosen to move one operand tells you whether
> the wrong value is being *computed* or *bypassed*.

This is the constructive companion to 209b (two readings, locally true, jointly incoherent). There
the incoherence was the signal; here the *arithmetic* is. Both replace a round of inference with
one calculation.

Boundary measured on four axes rather than argued: the two-statement chain is required; the
`* 31` in statement two is required; **`qword` is required** (all-`int64` is clean); and records,
nesting, cross-unit types, enum operands and being inside a function are **all irrelevant**.

### 210a — a signature name is a DISCOVERY COORDINATE, not a cause

It arrived as **`pxx-self_unitrec`** from the `--units 2` rung. **Units had nothing to do with
it** — nor the record, nor the nesting, nor the enum the generated program wrapped it in. The
signature is the statement kind at the first diverging checkpoint, which names *where the
divergence was noticed*, not *what is wrong*.

> *I nearly filed a cross-unit record ticket.*

Same family as 187a (a repro built from a failure message inherits that message's error) and
198a (`err[-1]` reporting a line adjacent to the truth) — but here the misleading name is
generated by the fuzzer's own taxonomy, so it arrives pre-labelled and looks like classification.
**A generated label is a claim about where, never about why**, and the reduction is what converts
one into the other.

And the correct conclusion about the rung, which is not the obvious one: **the multi-unit rung
earned its keep by reaching a program *shape* the single-file grammar had not produced — not
because the bug was about units.** That is an argument for *rotating rungs*, not for that rung.
A generator's job is to reach unvisited shapes; which knob got you there is incidental.

### 210b — do not merge two findings on a SUSPECTED common cause

Both `-O3` bugs are four-oracles-to-one, silent, and dense in `qword` arithmetic. pxx-a5 filed
them **separately** anyway:

> **Merging two findings on a suspected common cause is how one of them stops being tracked when
> the other is closed.**

One repro is eight lines with no `case`; the other is a 77-line nested `case`. Not the same
shape, and sameness *unestablished*. The disposal is the part to copy: instead of merging, the
new ticket's **acceptance criterion is to re-run seed 91162 after the fix and record the
answer** — which settles the question as a **measurement**, costs nothing, and cannot be
forgotten because it sits in the close condition.

Note the asymmetry that makes this the right default. Merging two tickets that turn out to be one
bug costs a duplicate close. Merging two that turn out to be *two* silently retires a live
defect — and it retires the one nobody was looking at.

Priority reasoning, stated and not inflated: **50 against the `case` ticket's 45, for one reason
only — eight lines makes acting on it cheap, and it may close both.**

### 210c — a record produced by the tool carries provenance a hand-edit cannot

The discovery run had no `--ledger`, so the entry was missing. pxx-a5's first instinct was to
splice the JSON in by hand; instead it **re-ran the seed through `pasmith_run --ledger`** —
five seconds, and the tool writes the args and the sha itself.

A hand-written record asserts a provenance; a tool-written one *has* one. The difference is
invisible in the file and total in what the file is worth, and five seconds is not a trade-off.

**Suspect discipline, worth noting alongside 207:** `ir_codegen.inc:6402` was named **as a
suspect and labelled as one** — `(OptLevel >= 3) and (op = tkStar) and (IRKind[left] =
