...
Recommend (1). Filed as a `bug-` rather than a `decide-` per CLAUDE.md's escape
rule — a compat finding that produces *silent wrong behaviour* is a bug in the
owning lane, not a parity nicety — but the direction above is the one thing
worth a human call before anyone starts.
