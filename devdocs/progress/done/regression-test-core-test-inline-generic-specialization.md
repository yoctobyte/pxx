...
> **Track guessed as P from the FAILING STEP** — line 1 of 2, `./compiler/pascal26 test/test_inline_generic_specialization.pas /tmp/test_inline_genspec26`, which names `test/test_inline_generic_specialization.pas`. Not from the job's name or its `src`: those describe what the job is ABOUT, and this job's recipe spans 2 source file(s). The ranker reads frontmatter, so this line — not the body — decides who works it; correct it if the guess is wrong.

> **origin/master has advanced 2 commit(s) since this sha.** Re-verify at current HEAD before acting — the callback is tagged to the sha that was tested, which may no longer be the state of the tree.

# regression: test-core#src:test/test_inline_generic_specialization.pas at b2a41d5f4fb9 in step 1/2, `./compiler/pascal26 test/test_inline_generic_specialization.pas /tmp/test_inline_genspec26` (auto-filed by twatch)

- **Type:** regression (auto-filed by Track T watcher, host seven, twatch `7327e547732c`).
  Untriaged.
- **Found:** 2026-09-06T16:20:47Z
- **Test source:** test/test_inline_generic_specialization.pas tools/expect_same.sh
- **Failing step:** line 1 of 2 of the job's recipe; it names `test/test_inline_generic_specialization.pas`.
  ```
  ./compiler/pascal26 test/test_inline_generic_specialization.pas /tmp/test_inline_genspec26
  ```

## Repro
`tools/testmgr.py --tier native --job 'test-core#src:test/test_inline_generic_specialization.pas'` at b2a41d5f4fb93afe09b23d1b7ee3999eaa906523
...
## Log tail
```
pascal26:36: error: expected 'begin' before 'MaxOf_Char'
(tail)
pascal26:36: error: expected 'begin' before 'MaxOf_Char'
  near: b := tmp ; end ; >>> MaxOf_Char as MaxCh 
...
`test_generic_func` — live there. The fixture eb2447470 added is an fpc
