...
owner: frankA
blocked-by: []
summary: "FIXED (5bea302b5). A cast to a named ARRAY type was not recognised at all: `TArr(aa)[1]` gives `undefined variable (TArr)` where fpc 3.2.2 compiles it. Swept every type kind a named cast can target and arrays are the ONLY gap — record, string alias, pointer alias, class, integer alias, enum, set and procedural type all worked. The cast dispatch consulted FindTypeAlias (plus the lazy builtin-pointer names) and never FindArrayType, which the SizeOf path two hundred lines away does consult. Not a postfix bug: the name never resolved, so it failed before any suffix was considered. Fixed by minting a pointer alias over the array row as a THIRD fallback (EnsureArrayPtrAlias) and adapting the ENDS — AN_ADDR under the operand, AN_DEREF over the AN_PTR_CAST — so the tree from there on is byte-for-byte what PArr(@aa)^ produced, adding no sixth postfix walker. Validating that target path before reusing it turned up two silent bugs IN it, both in pin v403 and both about to be inherited: the PChar adapter overwriting the pointee array row, and two of the three AN_INDEX-over-AN_DEREF minting sites never subtracting the low bound, in the read AND the write face."
---
