...
- **Slice 2b**: ✅ DONE (2026-07-04) — landed behind `-O3`, then **promoted to
  `-O2`** once proven (user call). Straight-line multi-statement bodies with
  scalar ordinal locals + single Result (`t:=a+b; Result:=t*t`). Retains the whole
  AN_SEQ chain with param/local/Result → `AN_INLINE_PARAM`/`AN_INLINE_LOCAL`/
  `AN_INLINE_RESULT` placeholders (`TryRetainInlineStmtBody`); at the call site
  allocs fresh caller locals + a Result temp, clones + lowers the statements,
  returns a load of the Result temp. Safety: straight-line only, all locals
  scalar, Result never read, read-before-write guard, slice-3 arg temps.
  Validated O0-vs-O2 across 500+ programs + all 4 cross targets; -O2
  self-fixedpoint byte-identical; test-opt + cross -O2 gated. Transparent
  downstream (B/C/D compile at -O0 where inline never fires).
  **Remaining slices moved to [[feature-inline-nonleaf-and-branch-locals]]:**
  slice 2c (if-then-else *with* locals — branch-aware assigned-before-read) and
  slice 4 (non-leaf / depth-budget inline).
