...
> **Track guessed as B from the FAILING STEP** — line 1 of 2, `stable_linux_amd64/default/pinned -Fulib/rtl/platform/posix test/lib_dns_multins.pas /tmp/lib_dns_multins`, which names `test/lib_dns_multins.pas`. Not from the job's name or its `src`: those describe what the job is ABOUT, and this job's recipe spans 2 source file(s). The ranker reads frontmatter, so this line — not the body — decides who works it; correct it if the guess is wrong.
...
- **Failing step:** line 1 of 2 of the job's recipe; it names `test/lib_dns_multins.pas`.
  ```
  stable_linux_amd64/default/pinned -Fulib/rtl/platform/posix test/lib_dns_multins.pas /tmp/lib_dns_multins
  ```
