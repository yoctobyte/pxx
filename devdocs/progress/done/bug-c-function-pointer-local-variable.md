...
- 2026-06-27 - DONE. ParseCLocalDeclAST: after the leading `ParseCDeclType`,
  when `CTypeFnPtrName <> ''` (inline `RET (*name)(params)` declarator, name
  already consumed) allocate the local under that name as a callable 8-byte
  pointer carrying `CTypeProcSig`, parse the optional `= init`, eat `;`, Exit.
  Mirrors the ParseCStructInto member branch. Test cfnptr_local_b95.c (exit 42,
  indirect call through the local). Self-host byte-identical; full `make test`
  green.
