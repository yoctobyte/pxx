...
summary: "DONE. i386 --emit-obj output was POSITION-DEPENDENT -- every .text relocation absolute R_386_32, so `-Wl,-z,text` refused the link and a PIE got DT_TEXTREL. Now ZERO absolute .text relocations on BOTH frontends (C subject 486 PC-relative, Pascal subject 502), both link under `gcc -m32 -pie -Wl,-z,text`, neither carries DT_TEXTREL, both run correctly -- all asserted in test-emit-obj as separate rows, with test/i386_pcrel_globals.c executing every converted shape. i386 has no [eip+disp32] ADDRESSING MODE, so the anchor is an instruction: a per-body call/pop parked in a frame slot, or materialised inline (also 6 bytes) where there is no frame. NO GOT -- our .data/.bss symbols are section-local, so the displacement is a link-time constant. Each reference loads the anchor into a register provably free AT THAT SITE (its own destination for a load; a push/pop scratch for a store; eax at a cdecl call, where the wrapper would break the already-pushed arguments), so this work contains no liveness audit anywhere. Addend is symOffset + (fieldPos - anchorPos), per site. Shapes: ModRM loads, moffs loads, address-as-immediate (-> lea), stores, SIB, plus ProcAddrFix and DynCall, which needed their own emitters because they never pass through EmitGlobRef/EmitDataRef. C7, 66 A3 and `mov [glob], esp` could not be rewritten in place -- the instruction is split between the caller and the emitter -- so three whole-instruction helpers emit them. Four defects found by RUNNING, not reading; method-index face 242 came out of it, and so did the finding that BOTH pre-existing gates passed against a deliberately +0x30000000 addend."
---
...
## Log
- 2026-09-01 — resolved; this names the commit that carried the resolve, which is not always the one that carried the change — commit 69075ffaf.
