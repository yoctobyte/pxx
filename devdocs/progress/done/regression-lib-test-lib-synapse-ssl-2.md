...
the name is gone from the only two places such a build can look. For lib_synapse_ssl the names
...
the log tail above; apply the fix and it prints `version=ok` and `SYNAPSE-SSL OK`.
