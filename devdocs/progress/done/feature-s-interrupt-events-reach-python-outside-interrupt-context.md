...
found-by: frankb-8e
blocked-by: []
summary: "DESIGN SETTLED BY THE OWNER 2026-09-22. BOTH HALVES NOW LAND: the Pascal PUMP (lib/rtl/interrupts.pas -- queue, drain at the blocking points the RTL already owns, re-entrancy refusal, per-drain budget, ring-full policy, iterator; platform.PalPendingDrain installed on first registration so a program that registers nothing keeps its behaviour exactly) AND the PYTHON surface (`import interrupts`, on_event/events/poll/pending/dropped/delivered/push, events() yielding a class with NAMED fields per mimic_shutil.terminal_size). Python never runs in interrupt context BY CONSTRUCTION, not by discipline: IntPush is the only thing an ISR calls and it holds no callable, so there is no path from a trap to the handler table. Acceptance is ORDERING (`after-push delivered 0` -- nothing delivered at push time) and COUNT (pushed == delivered + dropped), never `the callback ran`, which passes on an implementation that runs it in trap context. ONE COMPILER LINE CAME WITH IT: a bare NilPy import of a Pascal unit is refused unless the name is in PyRtlUnitServesPython (pasparser_proc.inc), so `interrupts` was added there on that function's own stated criterion. THE NILPY TEST ROW IS THEREFORE INERT UNTIL A PIN CARRIES THAT ENTRY and uses ./$(COMPILER) -- move it to $(PXX_STABLE) once one does; the Pascal row already passes under the pin. COST RECORDED, NOT HIDDEN: `uses interrupts` costs ~300 KB because the Python surface names pylib and pylib cannot be dropped by DCE -- measured x86-64 23,097 -> 320,881 B; the cost is pylib's (a bare `uses pylib` is 316,004 B) and no arrangement of the file is cheaper, and no split resolves because `import X` binds unit X before mimic_X. THE GPIO EDGE SOURCE EXISTS AND RAN ON AN ESP32-S3 BOARD (2026-09-24): espgpio.pas on_rising/on_falling/on_change arm a pin, and its ISR calls only IntPush; examples/esp32/gpio-edge-s3 passes ordering and count on silicon (feature-esp-gpio-and-adc-callback-slices). The ring was made safe for an ISR producer in the same commit (a producer-owned tail and an atomic count). MEASURED with a core-1 producer (test/esp_board_gpio_ring_stress.pas): exact over ~672k edges, while the old ring lost 2,503 and misordered 3,667. THE HIDDEN LOOP IS BUILT (2026-09-24, frankH): interrupts' finalization serves a program after its main body ends WHILE a handler is registered AND a live source is open (espgpio opens one per armed pin, espadc on start; the synthetic push is not a source). Desktop: nothing opens a source, so a script importing interrupts exits -- test/test_interrupt_desktop_script_exits.npy under timeout, whose positive control (dropping the source condition) hangs at rc 124. Mechanism: test/test_interrupt_hidden_loop.pas serves three events after 'main-end' and exits when the handler closes the source; -dNO_SOURCE delivers none. ESP: test/esp_board_hidden_loop.npy on the S3 board is served 20 ADC frames after falling off the end and stops the ADC from the handler. Nothing is left open on this ticket."
---
