...
the name is gone from the only two places such a build can look. For lib_synapse the names
...
the log tail above; apply the fix and it prints its eight expected rows (`b64=SGVsbG8sIFdvcmxkIQ==` through `cli-got=pong`).
