...
found-by: frankS
blocked-by: []
summary: "FIXED 2026-09-22 (frankb-8e, 5fccc890a) -- AND BOTH TURNED OUT TO BE MEMBERS OF A THREE-INSTANCE FAMILY WHOSE THIRD MEMBER WAS LIVE. (1) the raw `xtensa_call8(ExcLongJmpAddr - CodeLen)` in exception_emit.inc now goes through EmitXtensaCall8ToCode and records its CodeRef. THE PRESCRIPTION BELOW NAMED THE WRONG HELPER and following it would have shipped a live bug: EmitXtensaCallToCode is the CALL0 one, the stub is entered with a2 = &jmpbuf and ends in RETW, and the site loads a10 -- so CALL0 passes the argument in the wrong register and retw rotates no window back. It also had a second half nobody had seen: `xtensa_beq(a2, a4, 9)` hardcoded a 3+3+3 byte count, and the helper may widen to a literal slot, so the branch is now patched from CodeLen like the five sibling arms of the same procedure -- it was the only one of six counting bytes. (2) IREmitCodeCall's tail now REFUSES on riscv32/xtensa and names the helpers that exist, instead of emitting E8 rel32 onto a target with no such opcode. Verified not born red on both targets, bare and --platform=posix, --dce and --no-dce. Output is byte-identical before and after on the windowed xtensa route, which is the point -- both were latent. THE CONTROL THAT NEARLY CERTIFIED NOTHING: the first byte-comparison used --target=xtensa --platform=posix WITHOUT --xtensa-abi=windowed, which takes the Call0 arm at :535 and never reaches the edit; an Error probe in the arm found it, and --esp-profile=bare REFUSES the windowed ABI outright so no bare build reaches it either. Isolation guards the run, not the route. The third, LIVE member was EmitCodeAbsToRdx -- see bug-a-a-pascal-hello-world-is-63kb-after-emission-size-dce."
---
