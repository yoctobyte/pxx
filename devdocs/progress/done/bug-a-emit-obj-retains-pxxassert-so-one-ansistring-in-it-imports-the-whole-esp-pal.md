...
owner: ""
blocked-by: []
summary: "RESOLVED 2026-09-22 (frankh-c0): BOTH REMAINING ITEMS ARE CLOSED AND THE OVER-IMPORT IS GONE BY DEFAULT. Item (1) -- `--dce` not on by default under `--emit-obj` -- landed at 96366af9b, at the third attempt, with both prior walls fixed and in done/ (30898eef7 for the i386 two-object crash, ec8ce7192 for the GNU-ld IRAM segfault). Measured at HEAD on this ticket's own subject, xtensa `--emit-obj test/test_emit_obj.pas`, default vs `--no-dce` on ONE binary: relocations naming ESP-IDF symbols (lwip_*, vTaskDelay, esp_timer_get_time) go 24 -> 0, total relocations 457 -> 212, object 387304 -> 55036 B. ESP-IDF UND symbol-table entries are 22 EITHER WAY, which is not a shortfall but the exact thing this ticket predicted when it moved its ratchet off UND counts onto relocations on 2026-09-18: DCE removes CODE, not symbol entries, so a UND assertion cannot see this fix and would have sat green through it. Item (2) -- the ratchet counting the wrong quantity -- was already done on 2026-09-18. POPULATION for the numbers above, because they are not comparable to the ticket body's older ones without it: one object, target xtensa, default ABI, test/test_emit_obj.pas, compiler 3854783c605a at 96366af9b; ESP-IDF rows selected by grep on lwip_|vTask|esp_timer|rmdir|xQueue|xTask|pthread, which is a WIDER set than the shim's own, so 22 here and the body's 20 are two measurements and not a regression. riscv32 stays clean (UND 3). WHAT IS NOT FIXED AND IS NOT THIS TICKET: retention is still PER-UNIT -- naming `platform` at all still costs the whole unit -- and the layer that repairs that is feature-a-pull-builtinheap-on-demand-instead-of-predicting-it. `--dce` now removes the consequence by default; the prediction that pulls the unit in the first place is untouched. NOTE FOR ANYONE RE-RUNNING THE TIER: the emit-obj tier currently dies EARLIER than these rows, at test_shared_lib.c, on an unrelated HEAD regression (bug-c-a-c-file-compiles-to-an-object-only-if-it-happens-to-contain-a-pascal-keyword, p90), so the ratchet rows above were verified BY HAND rather than through the tier, and the tier cannot confirm them until that is fixed."
---
...
## Log
- 2026-09-22 - item (1) DELIVERED: `--emit-obj` now defaults `--dce` on - commit 96366af9b.
  Third attempt; both prior walls fixed and closed first (30898eef7, ec8ce7192).
- 2026-09-22 - verified BY HAND, not through the tier, because the emit-obj tier dies earlier
  on an unrelated C regression filed the same session at c3fca19bd. Saying so rather than
  quoting a tier row that never ran: xtensa ESP-IDF relocations 24 -> 0 by default,
  object 387304 -> 55036 B, ESP-IDF UND 22 either way exactly as this ticket predicted.
- 2026-09-22 - resolved. The per-unit retention MECHANISM is untouched and is not this
  ticket's: see feature-a-pull-builtinheap-on-demand-instead-of-predicting-it.
