...
owner: frankS
blocked-by: []
summary: "RESOLVED 2026-08-31. ALL SEVEN targets build the compiler (i386, aarch64, arm32, riscv32, wasm32, x86_64, and xtensa with `--platform=posix --xtensa-long-calls`), and the i386, aarch64 and riscv32 cross-built compilers each RUN under qemu and emit a working binary for their own architecture -- but only when told `--target=<self>`, because the compiled-in default target is x86_64 whatever the host arch is. ROOT CAUSE of the last defect, arm32's heap corruption (frankA, 5454ef402): EmitLoadVar/EmitStoreVar on arm32, riscv32 and xtensa sized a variable access with TypeSize(elementType), so a dynamic array's POINTER-sized handle slot got a one-byte access -- the prologue `strb` cleared only the low byte, the cleanup released a handle made of stack garbage, and the refcount decrement landed inside an already-freed block. aarch64 had carried the guard all along, which is why it was the silent control. Confirmed here against BOTH original symptom axes, not the reduced repro: environment pad 8/8 clean, output path length 7/7 clean with zero errors -- the latter is the WRONG-ANSWER face (four bogus `undefined variable (PXX_KIND_LEGACY)` errors blaming correct source) and it is gone at exactly the lengths that produced it. Sibling check done: every backend with the shape now has the guard. TWO THINGS DELIBERATELY NOT CLOSED HERE. The 32-bit hosted SELF-build still dies -- 14 arenas of 256 MiB succeed and the 15th returns ENOMEM, unchecked, on arm32 AND on i386 running NATIVELY with no emulator -- which is [[bug-a-pxxalloc-does-not-check-the-mmap-return-so-oom-arrives-as-an-anonymous-segv]] (renamed 2026-08-31), not this one; append nothing about it here. Its appetite half turned out NOT to be 32-bit-wide: it is [[bug-o-the-in-place-string-append-is-x86-64-only-so-every-other-backend-is-quadratic]], which hits aarch64 too and is merely fatal on 32-bit first. And one observation stays unexplained rather than folded in: a sampled bin head decoding to the ASCII bytes `Char` does not end in 0x00 and so is not itself a stale handle -- consistent as a downstream effect, not demonstrated. The title was false as written and false when filed: wasm32 built all along."
---
...
[[bug-a-pxxalloc-does-not-check-the-mmap-return-so-oom-arrives-as-an-anonymous-segv]]
