...
## 2026-09-22 — THE COST SIDE IS **UNMEASURED**, NOT ZERO, AND MY `13%` WAS MEANINGLESS RATHER THAN WEAK

**This is the sixth and last population error in this ticket, and it is the one
that nearly went out the door as a recommendation.** Twice above I put an honest
bound on the nine-of-nine control — *"≈13% assuming independence, a floor not an
estimate"* — and both times I was bounding a quantity that **does not exist**.

**What was missing was not rigour, it was one prior question:** are those
fifteen rates *rates at all*?

```
row                                         reds  longest consecutive run  last red      still red at the end?
size_canary.py                               189            158            2026-09-10    no - ENDED
install_lib_candidates.sh                     107             -             2026-09-11    no - ENDED
test_libwriteln_parity.pas                     86             86            2026-09-06    no - ENDED
... all 14 rows with any reds                                                             no - ENDED
```

**Every one of the fourteen had already ENDED before seven's last report, and
none was red in its final twenty.** `test_libwriteln_parity.pas` is 86
consecutive reds **inside one day**. These are single past **episodes**, not
per-run probabilities, so:

- `P(all nine pass) ≈ 13%` is not weak evidence. There is no `p`. It is
  **meaningless**, and it read as careful precisely *because* I had hedged it.
- The follow-up ticket I filed on the strength of it — *"the fifteen reversed
  rows are the real goal-1 backlog"* — was **rejected by my own next
  measurement, twenty minutes after I wrote it**, and is in `rejected/` because
  the report was **wrong**, not unimportant.

**THE CORRECTION TO THE OWNER-FACING SENTENCE, which is what matters here.**
"The cost of upgrading borg collapses" is *not* what fourteen closed episodes
establish. What they establish is that **nothing in the archive evidences a
cost** — and absence of evidence across fourteen finished episodes says nothing
about the next run. frankuser is carrying this and will carry the hedged
wording, not a stronger one:

> *borg's row fails 549 of 550; nothing in the archive says the upgrade would
> break anything; the cost side is **unmeasured** rather than demonstrated
> absent.*

One direction, honestly hedged, no design fork. That is a sentence he can act on
without being misled about how much is known.

### The recurrence is the transferable part, and it is TWO seats, not one

**The lesson was in hand, in this session, and stated out loud between us — and
it transferred to neither seat's next table.**

1. Hours earlier, frankuser asked of a different table *"are the sixteen greens
   CLUSTERED or SPREAD?"* and **dissolved a 2.6x of mine** on the spot.
2. I then built the reversals table and **did not ask my own version of that
   question**, filing a p60 ticket instead.
3. **And frankuser, handed that table, proposed *"run `size_canary` ten times
   and see whether it really sits at 52.4%"*** — a repeat experiment, which
   *presumes a per-run probability exists*. It never asked whether one did. Its
   own account: *"it is not one seat failing to carry a lesson across a session
   — it is two seats, in the same session, with the lesson explicitly stated
   between them, both failing to transfer it to the next table."*

**Instance 3 forecloses the obvious objection** — that somebody merely needed to
be more careful. The seat that *supplied* the question failed the same way when
the numbers were in front of it. The second table does not look like the same
mistake; **it looks like new data.**

**So the fix is in the tool and not in the reader.**
`tools/tstate_toolchain_reversals.py` now prints `[longest run N, last red
<date>]` beside every rate, so the shape arrives with the number and nobody has
to know to ask. It **annotates rather than refuses**, which is the honest limit:
it will still print `52.4%` on the same line as the `158` that contradicts it.
Written up as a playbook section with its own remaining half named.

**And the general form, across three of tonight's findings:** clustered-versus-
spread on the greens, step-versus-gradient on the wall, episodes-versus-rates on
the reversals. **All three are one time-ordered series collapsed into a single
number, and all three read as clean findings.**

### Re-measured the same hour, and the output now separates live from closed BY ITSELF

**Two things, one of which moves a number quoted throughout this ticket.**

**1. The subject row is now `550/551`, not `549/550`.** Both figures are correct
about their own population and **neither refutes the other** — one more 8.2.2
report arrived and it was another red, so the rate is unchanged at 99.8%. The
population for every 8.2.2 count in this ticket is *"native/full reports since
`2026-09-04T16:54:32Z`"*, which grows on its own; quote the count with the
population or it is not requotable.

**2. THE `last red` COLUMN DISCRIMINATES LIVE FROM CLOSED WITHOUT ANY
STATISTICS, AND IT DID SO ON THE FIRST RE-RUN.** Every reversed row's red count
is **byte-for-byte what it was** — 189, 107, 86, 65, 64 — with `last red`
between 2026-09-06 and 09-11. The subject row's count **grew**, with
`last red 2026-09-22`:

```
c_crtl_wait.c        8.2.2  550/551  99.8%  [longest run 549, last red 2026-09-22]   <- LIVE
size_canary.py      10.2.1  189/361  52.4%  [longest run 158, last red 2026-09-10]   <- CLOSED
test_libwriteln…    10.2.1   86/361  23.8%  [longest run  86, last red 2026-09-06]   <- CLOSED
```

**A closed episode's count is frozen; a live phenomenon's count moves.** So
re-running the census twice a fortnight apart is itself the cheapest possible
test of whether a rate is a rate — and it needs no clustering analysis at all,
only the same command twice. That is the sharper version of the playbook tell
*"the last event is old relative to the window's end"*, and it is worth more
than the tell because it cannot be misread: **the number either changed or it
did not.**
