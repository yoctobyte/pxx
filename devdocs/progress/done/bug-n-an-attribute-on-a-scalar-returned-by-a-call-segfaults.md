...
owner: frankh-c0
summary: "FIXED 2026-09-22. `mk().foo` on a scalar SEGFAULTED (rc 139) where CPython raises AttributeError. PyMakeDynAttrGet picked its route by asking 'is the receiver a VARIANT' while its own header AND pydynattr_get's body both state the contract as 'is it statically a CLASS' -- the same question until the receiver is STATICALLY SCALAR, which is exactly what a call result is, because a def's return type infers to tyInt64/tyDouble/tyBoolean/tyAnsiString rather than staying dynamic. PyMakeObjPtr then handed the integer 5 to pydynattr_get as an object pointer and ClassName dereferenced it. Comment vs code, decided for the comments. BROADER THAN THIS TICKET RECORDED: it listed int and str; float and bool crash too, all four the same predicate. THE PRESCRIBED INSTRUMENT WAS WRONG and that is the transferable part -- the ticket said to tag every AllocNode(AN_FIELD) and diff the counts, and NO AN_FIELD is involved, which is also why its earlier PyParseClassRecordSelectors probe fired zero times for all six rows; PXXDBG=a.ast answered in one command. Nine rows now byte-identical to CPython including the receiver type name; the fixture SEGFAULTS under pin v416 as its positive control and is wired into test-nilpy, which is an explicit list and not a glob. SIBLING, filed separately as bug-n-a-dynamic-attribute-store-on-a-scalar-variant-segfaults p70: the SETTER twin is independently broken, because pydynattr_set_v tells apart only the class-reference tag while pydynattr_get_v checks properly and says in its own comment why -- one concept, two runtime entry points, different beliefs about the same population. NOT THIS TICKET and still open: `(5).foo` and `mk().foo = 1` both fail to PARSE -- loud, no wrong value."
---
