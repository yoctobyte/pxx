...
blocked-by: []
summary: "FIXED 2026-09-24 (frankS), both halves. The MEMBER half landed earlier: a member taken from a type of unknown layout is refused. The DECLARATION half: a type with no definition in scope had size 0 rather than being refused an object, so `struct nosuch v;` defined a zero-byte object and `sizeof` answered 0 (memset became a no-op, malloc asked for 0 bytes). Now refused with gcc's meaning. CRefuseIncompleteObject covers block-scope objects (scalar, array, static, typedef'd) and CRefuseIncompleteSizeof covers sizeof(type). File-scope objects are QUEUED in pass 1 and refused by CCheckIncompleteGlobals once the whole file has been seen, because a tentative definition completed later in the file is legal. Still accepted, as in gcc: a pointer to an incomplete type, block and file `extern`, and the GNU empty struct (sizeof 0). Kind 2 (a body pxx declined to lay out) is refused at member access only and is not touched here. FOUND ON THE WAY AND FIXED IN THE SAME CHANGE: a FILE-SCOPE declarator list gave every declarator the FIRST one's type, so `int x, *p;` made p an int (`*p` segfaulted) and `int *p, x;` made x a pointer. The block-scope path always re-derived per declarator, and file scope now does the same. Guards: test/test_c_incomplete_object_refused.sh (16 rows, verdicts from gcc -c; the pinned compiler fails all 9 refuse rows) and test/test_c_global_declarator_list_types.c (gcc output; the pinned compiler differs). Census of 997 C files (zlib, lib/crtl/src, c-testsuite single-exec, and the top-level C files under test/), compile rc pinned v421 against new: exactly two changes, both expected (00053 and the tag-scope fixture now compile). The census cannot see a codegen change, which is what c-testsuite 220/220 on five targets is for.""
---
...
## 2026-09-24 (frankS): the declaration half, fixed

Rows (gcc verdicts measured with `gcc -c`, pxx at the final binary, sha256
5b8c484b2272...), all in test/test_c_incomplete_object_refused.sh:
- REFUSED: local, local array, local static, a typedef'd object, sizeof(type),
  a block-local forward shadowing a complete outer tag, a global, the second
  declarator of `struct nosuch *b, a;`, and a static global.
- ACCEPTED: a pointer, sizeof of a pointer, block extern, file extern, a
  tentative definition completed later, the GNU empty struct, and pointer-only
  declarator lists.

The `struct nosuch *b, a;` row is what exposed the declarator-list bug: `a` was
never refused because at file scope it had the first declarator's POINTER type.
The ordinary spellings showed the same defect: `int x, *p;` segfaulted on `*p`,
while the same line in a function body was right. The declarator-list fix
covers that too.

The C driver's header comment said pass 2 "reserve[s] globals (idempotent)".
It does not: pass 2 skips them with SkipCDeclToSemi. That is why the global
