...
> cross) consult `devdocs/developer/feature-matrix.md`, not this list. Kept below as
...
1. **Audit pass:** extend `devdocs/developer/feature-matrix.md` into a real
...
- 2026-06-17 — **audit pass done.** Built the per-target codegen parity matrix
  in `devdocs/developer/feature-matrix.md` (x86-64 / i386 / aarch64 / arm32),
...
- 2026-06-19 — **close-out pass: residue resolved or spun out; ticket DONE.**
  ...
  **State:** `make test` green (incl. byte-identical fixedpoint + `--threadsafe`
  ...
  → moved to `devdocs/progress/done/`.
