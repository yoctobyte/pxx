program test_esp_strmut;
{ Managed AnsiString mutation: s[i] write with copy-on-write (PXXStrUnique) and
  SetLength (PXXStrSetLen). Prints chars via PutC; matches the x86-64 oracle. }
...
begin
  s := 'PXX';
  s[1] := 'Q';        { sole owner -> mutate in place: Qxx }
  PutS(s);            { Qxx }
  t := s;             { shared (refcount 2) }
  s[2] := 'Y';        { COW: clone, s = QYx, t stays Qxx }
  PutS(s);            { QYx }
  PutS(t);            { Qxx }
  SetLength(s, 2);
  PutS(s);            { QY }
