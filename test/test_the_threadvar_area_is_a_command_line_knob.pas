program test_the_threadvar_area_is_a_command_line_knob;
{ The per-thread variable area is a fixed CAP whose size is chosen on the
  command line, and this program is 8 bytes over the default.

  385 Int64 threadvars = 3080 bytes against TLS_USER_BYTES_DEFAULT's 3072. So:

    pascal26 --threadsafe                      -> REFUSED, `t385: the
                                                  per-thread variable area is
                                                  full (3072 bytes)`
    pascal26 --threadsafe -dPXX_TLS_USER_4K    -> builds, and the rows below

  Both directions matter and only together. The refusal is what makes the cap
  honest — a program that outgrows the area is stopped at compile time, never
  silently given storage the reserved block does not contain, which is why the
  reserve can be baked in before parsing and why __pxxTlsBlockSize can fold to
  a literal. The acceptance is what makes the knob a knob rather than a
  documented constant: raising it needs no compiler rebuild.

  The last-declared name is the one written and read, deliberately: t385 is the
  declaration that sits past the default cap, so a knob that moved the
  DIAGNOSTIC without moving the STORAGE would still pass a test that touched
  t001.
  feature-a-the-threadvar-area-is-3072-bytes-of-bss-in-every-program-that-has-no-threadvar }
threadvar
  t001, t002, t003, t004, t005, t006, t007, t008, t009, t010, t011, t012, t013, t014, t015, t016, t017, t018, t019, t020,
  t021, t022, t023, t024, t025, t026, t027, t028, t029, t030, t031, t032, t033, t034, t035, t036, t037, t038, t039, t040,
  t041, t042, t043, t044, t045, t046, t047, t048, t049, t050, t051, t052, t053, t054, t055, t056, t057, t058, t059, t060,
  t061, t062, t063, t064, t065, t066, t067, t068, t069, t070, t071, t072, t073, t074, t075, t076, t077, t078, t079, t080,
  t081, t082, t083, t084, t085, t086, t087, t088, t089, t090, t091, t092, t093, t094, t095, t096, t097, t098, t099, t100,
  t101, t102, t103, t104, t105, t106, t107, t108, t109, t110, t111, t112, t113, t114, t115, t116, t117, t118, t119, t120,
  t121, t122, t123, t124, t125, t126, t127, t128, t129, t130, t131, t132, t133, t134, t135, t136, t137, t138, t139, t140,
  t141, t142, t143, t144, t145, t146, t147, t148, t149, t150, t151, t152, t153, t154, t155, t156, t157, t158, t159, t160,
  t161, t162, t163, t164, t165, t166, t167, t168, t169, t170, t171, t172, t173, t174, t175, t176, t177, t178, t179, t180,
  t181, t182, t183, t184, t185, t186, t187, t188, t189, t190, t191, t192, t193, t194, t195, t196, t197, t198, t199, t200,
  t201, t202, t203, t204, t205, t206, t207, t208, t209, t210, t211, t212, t213, t214, t215, t216, t217, t218, t219, t220,
  t221, t222, t223, t224, t225, t226, t227, t228, t229, t230, t231, t232, t233, t234, t235, t236, t237, t238, t239, t240,
  t241, t242, t243, t244, t245, t246, t247, t248, t249, t250, t251, t252, t253, t254, t255, t256, t257, t258, t259, t260,
  t261, t262, t263, t264, t265, t266, t267, t268, t269, t270, t271, t272, t273, t274, t275, t276, t277, t278, t279, t280,
  t281, t282, t283, t284, t285, t286, t287, t288, t289, t290, t291, t292, t293, t294, t295, t296, t297, t298, t299, t300,
  t301, t302, t303, t304, t305, t306, t307, t308, t309, t310, t311, t312, t313, t314, t315, t316, t317, t318, t319, t320,
  t321, t322, t323, t324, t325, t326, t327, t328, t329, t330, t331, t332, t333, t334, t335, t336, t337, t338, t339, t340,
  t341, t342, t343, t344, t345, t346, t347, t348, t349, t350, t351, t352, t353, t354, t355, t356, t357, t358, t359, t360,
  t361, t362, t363, t364, t365, t366, t367, t368, t369, t370, t371, t372, t373, t374, t375, t376, t377, t378, t379, t380,
  t381, t382, t383, t384, t385: Int64;
begin
  t001 := 11;
  t385 := 31;
  WriteLn('first=', t001);
  WriteLn('last=', t385);
  WriteLn('block=', __pxxTlsBlockSize);
end.
