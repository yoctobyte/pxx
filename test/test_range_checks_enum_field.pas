...
{ {$R+} slice 3: chr(out-of-range) into a Char dest and a record-FIELD static
  ...
  CAST is NOT checked (FPC doesn't either — oracle-verified). Subrange dests
  are a recorded residual (need subrange bounds metadata).
  feature-pascal-range-checks-r-plus. }
uses sysutils;
type TE = (e0, e1, e2);
     TR = record arr: array[1..3] of integer; end;
var e: TE; ch: char; i: integer; r: TR; caught: Integer;
begin
  caught := 0;
  {$R+}
  i := 9;
  ...
  i := 2;
  r.arr[i] := 7;
  writeln('ok ', r.arr[2], ' caught=', caught);
end.
