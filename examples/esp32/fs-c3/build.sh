...
grep -q " app_main" build/pxx_fs_c3.map && echo "app_main present in image map"
...
PYEFUSE
  ...
  got="$(tr -d '\r' < "$ser" | grep -E '^(PROBE|PAL):' || true)"
  want="$(printf 'PROBE: fat_mount rc=0\nPAL: open-create ok\nPAL: write n=7 (want 7)\nPAL: seek rc=0\nPAL: read n=7 (want 7)\nPAL: content-mismatches=0\nPAL: close rc=0\nPAL: open-EXCL=-38 (today: -38 unsupported)\nPAL: open-missing=-1 (today: -1, errno collapsed)\nPAL: VERDICT esp-pal-file-io-WORKS')"
  if [ "$got" = "$want" ]; then
    echo "OK   fs-c3 -- ESP PAL file I/O works on target; EXCL and errno gaps pinned"
    rm -f "$ser"
  else
    echo "FAIL fs-c3 baseline changed."
    echo "     If open-EXCL or open-missing moved, that is feature-pal-esp-posix-fd-semantics"
    echo "     landing and the new values are the point -- update want= deliberately."
    echo "     If the create/write/read rows broke, the PAL file path regressed."
    echo "want:"; printf '%s\n' "$want" | sed 's/^/    /'
    echo "got:";  printf '%s\n' "$got"  | sed 's/^/    /'
