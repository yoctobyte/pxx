...
# PXX -> ESP-IDF (ESP32-S3): the UART1 test. main/main.pas uses espuart and
# checks it with no wire: the UART's internal loopback, then TX and RX on one
# pad. It is compiled to a relocatable object, wrapped in an archive, and
# linked by the IDF build.
#
...
idf.py build
grep -q " app_main" build/pxx_uart_s3.map && echo "app_main present in image map"
