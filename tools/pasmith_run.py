...
def evaluate(oracle, src, workdir):
    ...
    The program's OWN directory goes on both compilers' unit paths, always. With
    --units that is how the unit set is found (feature-pasmith-multi-unit-
    programs); without it the directory holds no units and the flag is inert.
    Unconditional on purpose — a unit path that depends on a mode is a way for
    single-file and multi-unit runs to diverge for a reason that is not a bug.
    """
    out = os.path.join(workdir, "b_" + oracle.name.replace("/", "_"))
    udir = os.path.dirname(os.path.abspath(src))
    if oracle.kind == "fpc":
        # -FU keeps the .ppu/.o out of the source dir, so a second oracle cannot
        # link against units the first one compiled at a different -O level.
        objdir = os.path.join(workdir, "u_" + oracle.name.replace("/", "_"))
        os.makedirs(objdir, exist_ok=True)
        cmd = (["fpc", "-Mobjfpc", "-vw", "-Fu" + udir, "-FU" + objdir]
               + oracle.args + ["-o" + out, src])
    else:
        cmd = [PXX, "-Fu" + RTL, "-Fu" + udir] + oracle.args + [src, out]
...
def gen_args_for(a, seed):
    ...
    """
    if getattr(a, "units", 0):
        # A unit set is generated by its own path in pasmith.py, and none of the
        # single-file rungs apply to it — carrying them into the repro line would
        # promise a program the generator never emits.
        return ["--seed", str(seed), "--units", str(a.units)]
    args = ["--seed", str(seed)]
    for f in GEN_FLAGS:
...
def add_gen_flags(ap):
    # A MODE, not a rung: --units switches the generator to emitting a program
    # plus N units, and the single-file rungs do not apply to it. Off by default
    # — the single-file grammar is where the statement-level coverage lives; this
    # reaches the three order-dependent classes it structurally cannot express.
    ap.add_argument("--units", type=int, default=0,
                    help="emit a program plus N UNITS per seed instead of one "
                         "file: cross-unit exception matching, cross-unit symbol "
                         "resolution, initialization-section order")
...
def main():
    ...
    try:
        while True:
            ...
            rc, out, src = generate(gen_args, workdir, seed)
            if rc != 0:
