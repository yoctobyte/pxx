            ...
            if hits:
                ...
                if held:
                    ...
                    lines.append(
                        ...
                        f"({', '.join(shown)}{more}). MESSAGE THE HOLDER FOR "
                        f"CONTEXT — and if the holder is not a LIVE SESSION, "
                        f"take it: CLAUDE.md says `owner:` is ATTRIBUTION, not "
                        f"a claim, and a parked ticket with an owner is free to "
                        f"take. An owner names a SEAT, and a seat is a checkout, "
                        f"not a continuous session; check ListAgents before "
                        f"treating this line as a lock. (This text said DO NOT "
                        f"CLAIM IT until 2026-09-06, which contradicted "
                        f"CLAUDE.md and made a ticket held by a dead seat "
                        f"unliftable — the flag repeats forever and nobody may "
                        f"act on it. Measured: feature-pascal-corpus-oop held by "
                        f"frank-rust, no session, no process in the checkout, "
                        f"last commit six days old.) A long-lived hold is still "
                        f"where a stale prose "
                        ...
                        f"added later still fires. PLACEMENT CONFLICTS WITH "
                        f"MARKDOWN AND THIS CHECK READS LINES, NOT STRUCTURE: "
                        f"if the citation is in a TABLE ROW, a marker inserted "
                        f"between rows TERMINATES THE TABLE, and moving it "
                        f"clear of the table puts it outside the +/-2 window. "
                        f"The only correct placement there is to write the "
                        f"marker as a ROW OF THE TABLE. The obvious repair "
                        f"corrupts the document and this check goes green "
                        f"either way (frankD, 2026-09-06)"
                    )
                else:
                    lines.append(
