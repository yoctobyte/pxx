...
# Render a kanban grid + ready/leverage to docs/progress/BOARD.md.
# Generated on demand, never committed (gitignored) — the filesystem is the
# real index; a checked-in board would drift.
cmd_board_md() {
  local out="$PROG/BOARD.md"
  ...
  done
  {
    echo "# Progress board"
    echo
    echo "_Generated by \`tools/progress.sh board-md\` on $(date -u +%Y-%m-%dT%H:%MZ)._"
    echo "_Do not edit and do not commit — this file is gitignored; regenerate on demand._"
    echo
    local hdr="|" sep="|"
