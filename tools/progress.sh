...
for _st in urgent working unfinished backlog blocked done done-followup rejected; do
  mkdir -p "$PROG/$_st"
done
TRACK_FILTER=""
...
cmd_board() {
  ...
  local d n
  for d in urgent working unfinished backlog blocked done done-followup rejected; do
...
render_board_md() {
  local statuses=(urgent working unfinished backlog blocked done done-followup rejected)
  echo "# Progress board"
  echo
