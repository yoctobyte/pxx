...
for f in files:
    ...
    # Only the red sections name a row; anything else in the file (a repro
    # line, a failure-detail block) belongs to the same reds.
    redsec = "\n".join(
        seg for seg in re.split(r"^## ", txt, flags=re.M)
        if seg.startswith("STILL-RED") or seg.startswith("NEW-RED"))
    red = ROW in redsec
