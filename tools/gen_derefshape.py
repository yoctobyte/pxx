...
AXES
  ...
  face         WRITE through the spelling, and READ back through it

The third axis is not a third loop: every row does both, and its expected
output is the two halves joined by ' | '. It was added after the first two had
been green for a day -- see the note in tools/derefshape_matrix.sh for what it
caught the moment it was switched on.
...
KINDS = {
    # name      pointee element type       setup               index expr              write expr         read args ({B} = base)                       expected
    'fixdbl': ('array[0..3] of Double',     '',                 '[i]',                  '(i+1)*1.5',       "{B}[0]:0:2, ' ', {B}[3]:0:2",              '1.50 6.00'),
    'dyndbl': ('array of Double',           'SetLength(a, 4);', '[i]',                  '(i+1)*1.5',       "{B}[0]:0:2, ' ', {B}[3]:0:2",              '1.50 6.00'),
    'dynstr': ('array of AnsiString',       'SetLength(a, 4);', '[i]',                  "'v' + Chr(48+i)", "{B}[0], ' ', {B}[3]",                      'v0 v3'),
    ...
    'ptrelem':('array of PInteger',         'SetLength(a, 4);', '[i]',                  '@nums[i]',        "{B}[0]^, ' ', {B}[3]^",                    '10 13'),
    ...
    'md2':    ('array[0..1, 0..1] of Double','',                '[i div 2, i mod 2]',   '(i+1)*1.5',       "{B}[0,0]:0:2, ' ', {B}[1,1]:0:2",          '1.50 6.00'),
}
...
def main():
    ...
    for kn, (ty, mk, idx, wr, rd, exp) in KINDS.items():
        for sn, (decl, init, nam, needs_fn) in SPELL.items():
            ...
            # TWO reads, one row: through the plain array, then back through the
            # SPELLING itself. Every row used to read only via `a`, so the read
            # face of each spelling was uncovered by construction -- a matrix
            # can be a full product on its named axes and still be blind on an
            # axis it never named. `TP(raw)^[1,1]` in expression position was
            # a hard COMPILE ERROR while the identical write compiled and ran.
            L.append(f"  Write({rd.replace('{B}', 'a')});")
            L.append("  Write(' | ');")
            L.append(f"  WriteLn({rd.replace('{B}', nam + chr(94))});")
            L.append('end.')
            with open(os.path.join(D, f'ds_{sn}_{kn}.pas'), 'w') as f:
                f.write('\n'.join(L) + '\n')
            rows.append((f'ds_{sn}_{kn}', exp + ' | ' + exp))
