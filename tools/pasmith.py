...
class UnitSet:
    """A program plus N units — the one bug class a single-file generator cannot
    reach (feature-pasmith-multi-unit-programs).

    Three shipped bug shapes live only here, and all three are about ORDER:

      * exception class matching ACROSS units — `on E: TBase` failing to catch a
        descendant declared in a later unit (the b339 shape, cross-unit);
      * symbol resolution that depends on which unit a name came from;
      * initialization-section ordering.

    The last one needs care, or the test is unsound. Pascal initialises a unit's
    DEPENDENCIES before the unit itself, and only then follows the uses clause.
    So the units are emitted as a CHAIN — unit k uses unit k-1 — which pins the
    true initialisation order to u0, u1, ... uN-1 no matter what order the
    program's uses clause names them in. The program's uses clause is then
    shuffled from the seed. A compiler that initialises in uses-clause order
    instead of dependency order produces a different order log and diverges;
    a compiler that gets it right cannot be made to differ by the shuffle.
    That keeps this inside the generator's contract — only well-defined
    programs, no implementation-defined evaluation order.

    The checksum discipline is unchanged: one number out, so the oracle
    machinery needs nothing new.
    """

    def __init__(self, seed, nunits, trace=False):
        self.seed = seed
        self.n = max(1, nunits)
        self.trace = trace
        self.rnd = random.Random(seed ^ 0x5A17)   # own stream: adding --units
        self.sup = "pasmsup_%d" % seed            # must not shift the program's
        self.units = ["pasmu_%d_%d" % (seed, k) for k in range(self.n)]

    # ---- the support unit: the initialisation-order log -------------------
    def gen_support(self):
        L = ["unit %s;" % self.sup,
             "{ GENERATED by tools/pasmith.py -- seed %d. Do not edit. }" % self.seed,
             "{$mode objfpc}{$Q-}{$R-}",
             "interface",
             "var",
             "  ord_n: longint;",
             "  ord_log: array[0..63] of longint;",
             "procedure Note(k: longint);",
             "function OrdFold: int64;",
             "implementation",
             "procedure Note(k: longint);",
             "begin",
             "  if ord_n <= High(ord_log) then begin",
             "    ord_log[ord_n] := k + 1;",
             "    ord_n := ord_n + 1;",
             "  end;",
             "end;",
             "function OrdFold: int64;",
             "var i: longint; h: qword;",
             "begin",
             "  h := 1469598103;",
             "  for i := 0 to ord_n - 1 do h := qword(h * 1000003) xor qword(ord_log[i]);",
             "  OrdFold := int64(h);",
             "end;",
             # Every unit uses this one, so it is initialised before all of them
             # — which is the only reason the log can be trusted to be complete.
             "initialization",
             "  ord_n := 0;",
             "end."]
        return "\n".join(L) + "\n"
    ...
    # ---- unit k -----------------------------------------------------------
    def gen_unit(self, k):
        me, prev = self.units[k], self.units[k - 1] if k else None
        base = "EU%d" % (k - 1) if k else "Exception"
        uses = ["sysutils", self.sup] + ([prev] if prev else [])
        L = ["unit %s;" % me,
             "{ GENERATED by tools/pasmith.py -- seed %d. Do not edit. }" % self.seed,
             "{$mode objfpc}{$Q-}{$R-}",
             "interface",
             "uses %s;" % ", ".join(uses),
             "type",
             # The cross-unit exception chain: EUk descends from a class declared
             # in a DIFFERENT, earlier unit. Raising EUk and catching it as EUj
             # (j<k) is the b339 shape with a unit boundary through the middle.
             "  EU%d = class(%s);" % (k, base),
             "  TEnU%d = (eu%da, eu%db, eu%dc);" % (k, k, k, k),
             "  TRU%d = record" % k,
             "    a: longint;",
             "    e: TEnU%d;" % k]
        if prev:
            # A field whose TYPE comes from the previous unit: the layout of this
            # record cannot be computed without resolving across the boundary.
            L.append("    p: TRU%d;" % (k - 1))
        L += ["  end;",
              "var",
              "  gu%d: longint;" % k,
              "  ru%d: TRU%d;" % (k, k),
              "function FU%d(v: int64): int64;" % k,
              "procedure RaiseU%d;" % k,
              "function RFold%d: int64;" % k,
              "implementation",
              "function FU%d(v: int64): int64;" % k,
              "begin",
              "  FU%d := int64(qword(v) * qword(%d) xor qword(gu%d));"
              % (k, 1000003 + k * 7, k),
              "end;",
              "procedure RaiseU%d;" % k,
              "begin",
              "  raise EU%d.Create('u%d');" % (k, k),
              "end;",
              # A LOCAL accumulator, never the function's own name on the right
              # of an assignment: `RFoldk := f(RFoldk)` reads as "the result so
              # far" to FPC and provokes a bare-own-name warning from pxx, and a
              # construct whose meaning is arguable is exactly what a
              # differential generator must not emit — the divergence it would
              # produce is nobody's bug.
              "function RFold%d: int64;" % k,
              "var h: qword;",
              "begin",
              "  h := qword(ru%d.a) * 31 xor qword(ord(ru%d.e));" % (k, k)]
        if prev:
            L.append("  h := h * 31 xor qword(ru%d.p.a);" % k)
        L.append("  RFold%d := int64(h);" % k)
        L += ["end;",
              "initialization",
              "  gu%d := %d;" % (k, k + 1),
              "  ru%d.a := %d;" % (k, self.rnd.randint(1, 1000)),
              "  ru%d.e := eu%d%s;" % (k, k, self.rnd.choice("abc"))]
        if prev:
            # Reads a variable initialised by the PREVIOUS unit's initialization
            # section. If that section has not run yet, this reads zero and the
            # checksum says so — which is the ordering property, made observable.
            L.append("  ru%d.p.a := gu%d * 7;" % (k, k - 1))
        L += ["  Note(%d);" % k,
              "end."]
        return "\n".join(L) + "\n"
    ...
    def gen_program(self):
        ...
        self.rnd.shuffle(order)      # shuffled ON PURPOSE — see the class docstring
        L = ["program pasmith_%d;" % self.seed,
             "{ GENERATED by tools/pasmith.py -- seed %d. Do not edit." % self.seed,
             "  Reproduce with: tools/pasmith.py --seed %d --units %d --outdir DIR"
             % (self.seed, self.n),
             # Same self-describing contract as the single-file header: the
             # driver reads this line back (gen_args_of) to rebuild the identical
             # subject in trace mode. Without it localize() would regenerate this
             # seed as a SINGLE FILE — a different program that does not diverge.
             "  gen-args: --units %d }" % self.n,
             "{$mode objfpc}{$Q-}{$R-}",
             "uses sysutils, %s, %s;" % (self.sup, ", ".join(order)),
             "var",
             "  cs: qword;",
             "  i: longint;",
             ...
             "end;",
             "begin",
             "  cs := 14695981039346656037;"]

        # In trace mode every step prints the running checksum and carries a
        # `{ checkpoint N kind=K }` marker — the SAME contract the single-file
        # generator uses, so pasmith_run's localize()/checkpoint_kinds() name the
        # diverging statement here with no driver change, and the signature reads
        # e.g. pxx-vs-fpc_unitexc: the bug class, straight out of the kind.
        self._ck = 0
        ...
        def step(lines, kind):
            ...
            if self.trace:
                self._ck += 1
                L.append("  writeln(cs);   { checkpoint %d kind=%s }"
                         % (self._ck, kind))

        for k in range(self.n):
            step(["  Mix(FU%d(%d));" % (k, self.rnd.randint(1, 100000))],
                 "unitfn")
            step(["  Mix(gu%d);" % k], "unitvar")
            step(["  Mix(RFold%d);" % k], "unitrec")
        for k in range(self.n):
            # Catch on a base declared in an EARLIER unit than the one that
            # raises: every raise is caught by construction, so the program stays
            # well-defined whatever the compiler does with the hierarchy.
            j = self.rnd.randint(0, k)
            step(["  try",
                  "    RaiseU%d;" % k,
                  "    Mix(-1);            { unreachable: the raise always fires }",
                  "  except",
                  "    on E: EU%d do Mix(Length(E.Message) * 100 + %d);" % (j, k),
                  "  end;"], "unitexc")
        step(["  Mix(OrdFold);"], "unitord")
        step(["  for i := 0 to ord_n - 1 do Mix(ord_log[i]);"], "unitordlog")
        L += ["  writeln(cs);",
              "end."]
        return "\n".join(L) + "\n"

    def write(self, outdir):
        """Write the whole set. Returns the program's path — what the driver
        compiles; the units reach it through -Fu on that directory."""
        os.makedirs(outdir, exist_ok=True)
        for name, src in [(self.sup, self.gen_support())] + \
                         [(u, self.gen_unit(k)) for k, u in enumerate(self.units)]:
            with open(os.path.join(outdir, name + ".pas"), "w") as f:
                f.write(src)
        prog = os.path.join(outdir, "pasmith_%d.pas" % self.seed)
        with open(prog, "w") as f:
            f.write(self.gen_program())
        return prog
...
def main():
    ...
    ap.add_argument("--units", type=int, default=0,
                    help="emit a PROGRAM PLUS N UNITS instead of one file "
                         ...
                         "bug classes a single file cannot express: cross-unit "
                         "exception matching, cross-unit symbol resolution, and "
                         "initialization-section order.")
    ap.add_argument("--outdir",
                    help="directory for --units (the unit set + the program)")
    ...
    a = ap.parse_args()
    if a.units:
        # A separate generator, not a rung on the statement stream: the unit set
        # is about what lives in WHICH FILE, which the single-file Gen has no
        # representation for. Kept deliberately narrow — the point is the unit
        # boundary, not another grammar widening behind it.
        if not a.outdir:
            ap.error("--units needs --outdir (a unit set is several files)")
        prog = UnitSet(a.seed, a.units, a.trace).write(a.outdir)
        if not a.output:
            print(prog)
        return 0
    if a.wide:
